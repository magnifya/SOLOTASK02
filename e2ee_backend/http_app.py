...
class DeviceHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _route_device_inbox_lease_release(self, path: str) -> None:
        # Split on raw slashes only; a percent-encoded slash inside a
        # segment is part of the id, mirroring the other device routes.
    ...
    def _handle_device_inbox_lease_release(self, device_id: str,
                                           lease_id: str) -> None:
        # The release takes no request body; any non-empty body is a 400.
        try:
        ...
        if raw:
            ...
            return
        try:
            body, status_code = self.service.inbox_release_lease(
                device_id, lease_id)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
