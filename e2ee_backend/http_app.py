...
class DeviceHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _decimal_nonneg_param(self, query: Dict[str, list], name: str,
        ...
        refused. When *max_digits* is given, the digit string with its
        leading zeroes removed may not exceed that many digits (an all-zero
        value reduces to the single digit ``0`` and is then range-checked),
        bounding the value before ``int()`` is ever called. Sends a 400 and
        returns ``None`` when the parameter is repeated, empty, not a strict
        decimal integer, too long after zero stripping, or outside
        *minimum*..*maximum*; an absent parameter yields *default*.
        ...
        if max_digits is not None:
            ...
            if len(stripped) > max_digits:
                self._send_json(400, {
                    "message": f"{name} must be a decimal integer",
                    "field": name})
                return None
        value = int(text)
        if value < minimum or (maximum is not None and value > maximum):
