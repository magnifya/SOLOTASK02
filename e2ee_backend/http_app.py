...
class DeviceHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _handle_device_inbox_jobs(self, device_id: str) -> None:
        # The redelivery-job history query takes no request body: a
        # non-empty one is 400/request_body. Only single-valued
        # state/after/limit query parameters are accepted (defaults
        # all/0/100); anything else is 400/query. keep_blank_values so a
        # bare ``?foo`` flag is an actual (unknown) parameter rather than
        # being silently dropped, while a trailing ``?`` with no parameter
        # at all is accepted.
        query = parse_qs(urlsplit(self.path).query,
                         keep_blank_values=True)
        if any(name not in ("state", "after", "limit")
        ...
        if length > 0:
            ...
            return
        try:
            body = self.service.inbox_jobs(device_id, state, after, limit)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
        self._send_json(200, body)
