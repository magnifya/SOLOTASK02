...
class DeviceHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if path == _DEVICES_PATH:
            ...
            self._route_identity_rotate(path)
        elif (path.startswith(_DEVICES_PATH + "/")
              and path.endswith("/prekeys")):
    ...
    def _route_identity_rotate(self, path: str) -> None:
        # /v1/devices/{device_id}/identity-key/rotate
    ...
    def _route_add_prekey(self, path: str) -> None:
        # /v1/devices/{device_id}/prekeys
        suffix = path[len(_DEVICES_PATH) + 1:-len("/prekeys")]
        parts = suffix.split("/")
        if len(parts) == 1 and parts[0]:
