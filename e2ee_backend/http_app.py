...
class DeviceHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _route_POST(self) -> None:
        ...
        elif path == _INBOX_JOBS_PATH + "/ack-batch":
            self._handle_inbox_job_ack_batch()
        elif path == _INBOX_JOBS_PATH + "/renew-batch":
            self._handle_inbox_job_renew_batch()
        elif path == _INBOX_JOBS_PATH + "/release-batch":
    ...
    def _route_device_inbox_lease_renew(self, path: str) -> None:
        ...
        if (len(parts) == 4 and parts[0] and parts[1] == "inbox"
            ...
            self._handle_device_inbox_lease_renew(
    ...
    def _route_device_inbox_lease_ack(self, path: str) -> None:
        # {device_id}/inbox/leases/{lease_id}/ack — split on raw slashes
        # only; a percent-encoded slash inside an id segment is part of it.
        suffix = path[len(_DEVICES_PATH) + 1:-len("/ack")]
        parts = suffix.split("/")
        if (len(parts) == 4 and parts[0] and parts[1] == "inbox"
                and parts[2] == "leases" and parts[3]):
            self._handle_device_inbox_lease_ack(
                unquote(parts[0]), unquote(parts[3]))
        else:
            self._send_json(404, {"message": "device not found",
                                  "field": "device_id"})
    ...
    def _route_device_inbox_lease_complete(self, path: str) -> None:
        # {device_id}/inbox/leases/{lease_id}/complete — split on raw slashes
        # only; a percent-encoded slash inside an id segment is part of it.
    ...
    def _handle_device_inbox_wait(self, device_id: str) -> None:
        # The long poll takes no request body and only single-valued
        # limit/timeout_ms query parameters. Validation order is fixed:
        # non-empty body (request_body), any other parameter (query), then
        # limit, then timeout_ms. keep_blank_values so a bare ``?foo`` flag or
        # an explicit ``limit=`` is an actual (malformed) parameter; a
    ...
    def _handle_device_inbox_lease_renew(
        ...
        if payload is _BAD_REQUEST:
            return
        try:
            body, status_code = self.service.inbox_lease_renew(
    ...
    def _handle_device_inbox_lease_ack(
            self, device_id: str, lease_id: str) -> None:
        # The bulk ack takes no request body; any non-empty one is a 400.
        try:
            length = int(self.headers.get("Content-Length", "0"))
        except ValueError:
            self._send_json(400, {"message": "invalid Content-Length header",
        ...
        if length > 0:
            ...
            return
        try:
            body, status_code = self.service.inbox_lease_ack(
                device_id, lease_id)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
    ...
    def _handle_device_inbox_leases(self, device_id: str) -> None:
        ...
        # 400/request_body. Only single-valued state/after/limit query
        # parameters are accepted (defaults all/0/100); anything else is
        # 400/query. keep_blank_values so a bare ``?foo`` flag is an actual
        # (unknown) parameter rather than being silently dropped, while a
        # trailing ``?`` with no parameter at all is accepted.
        query = parse_qs(urlsplit(self.path).query,
                         keep_blank_values=True)
        if any(name not in ("state", "after", "limit")
               for name in query):
            self._send_json(400, {"message": "query parameters are not "
                                             "accepted",
                                  "field": "query"})
        ...
        if state not in ("all", "active", "expired", "released",
                         "completed"):
            self._send_json(400, {"message": "state must be one of 'all', "
                                             "'active', 'expired', "
        ...
        if length > 0:
            ...
            return
        try:
            body = self.service.inbox_leases(device_id, state, after, limit)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
        self._send_json(200, body)

    def _handle_device_inbox_jobs(self, device_id: str) -> None:
        # The job history query takes no request body: a non-empty one is
        # 400/request_body. Only single-valued state/after/limit query
        # parameters are accepted (defaults all/0/100); anything else is
        # 400/query. keep_blank_values so a bare ``?foo`` flag is an actual
        # (unknown) parameter rather than being silently dropped, while a
    ...
    def _handle_device_inbox_job_events(self, device_id: str) -> None:
        ...
        # is 400/request_body (checked first). Only single-valued after/
        # limit query parameters are accepted (defaults 0/100); anything
        # else is 400/query. after must be an ASCII decimal integer in
        # 0..2^63-1 and limit in 1..100; an invalid or repeated parameter
        # is 400 with that parameter's name. keep_blank_values so a bare
        # ``?foo`` flag or an explicit ``after=`` is an actual (malformed)
        # parameter, while a trailing ``?`` with no parameter at all is
        # accepted.
        try:
            length = int(self.headers.get("Content-Length", "0"))
        except ValueError:
        ...
        query = parse_qs(urlsplit(self.path).query,
                         keep_blank_values=True)
        if any(name not in ("after", "limit") for name in query):
            self._send_json(400, {"message": "query parameters are not "
                                             "accepted",
                                  "field": "query"})
            return
        ...
        if limit is None:
            return  # a 400 response was already sent
        try:
            body = self.service.inbox_job_events_page(
    ...
    def _handle_inbox_job_dispatch_batch(self) -> None:
        payload = self._read_json_request()
        if payload is _BAD_REQUEST:
            return
        try:
            body, status_code = self.service.inbox_job_dispatch_batch(payload)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
        self._send_json(status_code, body)
    ...
    def _handle_inbox_job_cancel_batch(self) -> None:
        payload = self._read_json_request()
        if payload is _BAD_REQUEST:
            return
        try:
    ...
    def _handle_inbox_job_complete_batch(self) -> None:
        ...
        if payload is _BAD_REQUEST:
            return
        try:
            body, status_code = self.service.inbox_job_complete_batch(payload)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
        self._send_json(status_code, body)

    def _handle_inbox_job_ack_batch(self) -> None:
        payload = self._read_json_request()
        if payload is _BAD_REQUEST:
            return
        try:
            body, status_code = self.service.inbox_job_ack_batch(payload)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
        self._send_json(status_code, body)

    def _handle_inbox_job_renew_batch(self) -> None:
        payload = self._read_json_request()
        if payload is _BAD_REQUEST:
            return
        try:
            body, status_code = self.service.inbox_job_renew_batch(payload)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
        self._send_json(status_code, body)

    def _handle_inbox_job_release_batch(self) -> None:
        payload = self._read_json_request()
        if payload is _BAD_REQUEST:
            return
        try:
            body, status_code = self.service.inbox_job_release_batch(payload)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
        self._send_json(status_code, body)

    def _handle_inbox_job_lease_status_batch(self) -> None:
        payload = self._read_json_request()
        if payload is _BAD_REQUEST:
            return
        try:
