...
class DeviceHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _route_POST(self) -> None:
        ...
        if path == _DEVICES_PATH:
            ...
            self._route_sync_ack_batch(path)
        elif path.startswith(_DEVICES_PATH + "/"):
    ...
    def _route_sync_ack_batch(self, path: str) -> None:
        # Split on raw slashes only; a percent-encoded slash inside the
        # segment is part of the device id, mirroring the revoke route.
        suffix = path[len(_DEVICES_PATH) + 1:-len("/sync/ack-batch")]
        if suffix and "/" not in suffix:
            self._handle_sync_ack_batch(unquote(suffix))
        else:
            self._send_json(404, {"message": "device not found",
                                  "field": "device_id"})
    ...
    def _handle_sync_ack_batch(self, device_id: str) -> None:
        payload = self._read_json_request()
        if payload is _BAD_REQUEST:
            return
        try:
            body, status_code = self.service.sync_session_ack_batch(
                device_id, payload)
        except ServiceError as error:
            self._send_json(error.status_code, error.to_body())
            return
