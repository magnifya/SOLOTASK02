...
#: A renew-batch mixed first-time items with replays of already committed
#: renewals; the batch is atomic, so a partial replay conflicts
...
class InboxLeaseRenewBatchError(Exception):
    ...
    offending item; items are prechecked in array order and the whole batch
    writes nothing. Reasons are the per-lease :class:`InboxLeaseError`
    renewal reasons (``lease_not_found`` -> 404/items[i].lease_id,
    ``lease_id_conflict`` / ``lease_unavailable`` ->
...
class DeviceStore:
    ...
    def inbox_lease_renew_batch(
        ...
        ``POST /v1/inbox-jobs/renew-batch``; *items* are ``(lease_id,
        renewal_id)`` pairs already validated for shape and per-field
        uniqueness (no ``lease_id`` or ``renewal_id`` repeats) by the
        service. Under the one store lock — shared with claims, releases,
        renewals, completions, acks, retries, redelivery jobs and
        revocation — the device is resolved first (unknown/revoked ->
        :class:`InboxLeaseError` ``device_unknown``/``device_inactive``,
        409/device_id), ahead of every item, before any item is decided.

        Every item is then prechecked in input order with exactly the
        single-lease renewal rules, the first failure raising
        ...
        Replaying the same renewal_id on the same lease is decided next:
        such an item skips the remaining state checks, as the single-lease
        entry does, however stale the request (the lease may since have
        expired, been released, completed or the device revoked).
        ...
        first-time lease is renewed in input order, each extending its own
        current effective deadline — the claim deadline or its last
        renewal's — by exactly :data:`INBOX_LEASE_SECONDS` seconds; a lease
        already released, completed or past its current effective deadline
        raises ``lease_unavailable`` (409/items[i].lease_id). The whole
        batch commits with one persistence notification (201, commit_seq +
        1; a durable write failure rolls every record back). Returns
        ...
        with self._lock:
            now = datetime.now(timezone.utc)
            # The device gate precedes every item decision, mirroring the
            # other inbox batch endpoints.
            device = self._find_device(device_id)
            if device is None:
                raise InboxLeaseError(INBOX_LEASE_DEVICE_UNKNOWN)
            if device.revoked:
            ...
            for index, (lease_id, renewal_id) in enumerate(items):
                ...
                # The replay decision precedes the lease-state check,
                # mirroring the single-lease renewal: an exact replay of the
                # same renewal_id answers the frozen first response, however
                # stale the request.
                replay_until: Optional[str] = None
                for renewal in hits[0][1].renewals:
                    if renewal.renewal_id == renewal_id:
                        replay_until = renewal.leased_until
                        break
                ...
                # released, completed or past its current effective deadline
            ...
            if all(replays):
                # Every item replays its committed renewal: answer the
            ...
            # Each first-time item extends its own current effective
            # deadline by exactly the lease lifetime; items touch disjoint
            # lease_ids (validated by the service), so the per-lease
            # extensions are independent even when the leases share
            # delivery records.
            results: List[Dict[str, Any]] = []
            for index, ((lease_id, renewal_id), hits) in enumerate(
                    zip(items, resolved)):
                current = self._lease_effective_deadline_locked(hits[0][1])
                # The active check above guarantees a parseable aware
                # deadline on every copy.
                if current is None:  # pragma: no cover - ruled out above
                    raise InboxLeaseRenewBatchError(
                        INBOX_LEASE_UNAVAILABLE, index)
                leased_until = (current + timedelta(
            ...
            # per-lease renewal commits (or rolls back) together and the
