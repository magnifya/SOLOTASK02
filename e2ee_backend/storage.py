...
#: A cancel-batch mixed first-time items with replays of already committed
#: cancellations; the batch is atomic, so a partial replay conflicts
#: (409/items[i].cancellation_id of the first replayed item).
REDELIVERY_JOB_CANCEL_PARTIAL_REPLAY = "partial_replay"
...
class RedeliveryJobCancelBatchError(Exception):
    ...
    offending item; items are prechecked in array order and the whole batch
    writes nothing. Reasons are the per-item
    :class:`RedeliveryJobError` cancel reasons plus ``partial_replay`` for
    a batch mixing first-time items with replays of committed
    cancellations.
    """
...
class DeviceStore:
    ...
    def _redelivery_job_apply_cancel_locked(
        ...
        """Cancel one ``pending``/``running`` job. Lock required.

        The caller guarantees *job* is cancellable (``pending`` or
        ``running``). A ``running`` job's current lease is released in the
        same transaction so its messages are claimable again (the lease
        stays as history; an already-released current lease keeps its
        earlier release timestamp); a ``pending`` job touches no lease and
        keeps ``lease_id`` null. Either way the job ends ``cancelled`` with
        *cancellation_id* and the UTC *cancelled_at* stamp.
        """
    ...
    def redelivery_job_cancel_batch(
        ...
        """Atomically cancel many pending/running jobs of one device.
        ...
        dispatch, leases, completions, acks and revocation — the device is
        resolved first (unknown/revoked -> :class:`RedeliveryJobError`
        ``device_unknown``/``device_inactive``, 409/device_id).
        ...
        ``job_id_conflict`` (409/items[i].job_id), a ``succeeded``/``failed``
        terminal job ``job_not_cancellable`` (409/items[i].job_id) and an
        already cancelled job under a different ``cancellation_id``
        ``cancellation_id_conflict`` (409/items[i].cancellation_id). An item
        replaying its own committed cancellation (same id on the same
        cancelled job) skips the remaining checks, as the single-job entry
        does.
        ...
        replayed item (409/items[i].cancellation_id). Otherwise each job is
        cancelled in input order via
        :meth:`_redelivery_job_apply_cancel_locked` — pending jobs keep
        ``lease_id`` null, running jobs release their current lease so the
        messages are claimable again — and the whole batch commits with one
        persistence notification (201, commit_seq + 1; a durable write
        failure rolls every job and lease back). Returns ``(results,
        status_code)`` with one three-key (``job_id``/``state``/
        ``lease_id``) view per item, in input order.
        """
        with self._lock:
            device = self._find_device(device_id)
            if device is None:
            ...
            for index, (job_id, cancellation_id) in enumerate(items):
                ...
                jobs.append(job)
                if job.state == REDELIVERY_JOB_CANCELLED:
                    # An exact replay of the committed cancel skips the
                    # remaining per-item checks (mirroring the single-job
                    # entry) and is only classified here; a different id on
                    # the cancelled job conflicts.
                    if job.cancellation_id == cancellation_id:
                        replays.append(True)
                        continue
                    raise RedeliveryJobCancelBatchError(
            ...
            if all(replays):
                # Every item replays its committed cancellation: answer the
                # current views with 200 and write nothing (no persistence
                # notification, no commit_seq advance).
                return [self._cancel_batch_item_view(job) for job in jobs], \
                    200
            if any(replays):
                ...
                raise RedeliveryJobCancelBatchError(
                    ...
                    replays.index(True))
            # One cancellation timestamp for the whole batch: every item
            # commits in the same locked transaction, mirroring the single
            # cancel's timespec="microseconds" rendering.
            now = datetime.now(timezone.utc)
            cancelled_at = now.isoformat(timespec="microseconds")
            ...
            self._notify_change()
            return [self._cancel_batch_item_view(job) for job in jobs], 201

    @staticmethod
    def _cancel_batch_item_view(job: RedeliveryJob) -> Dict[str, Any]:
        """One cancel-batch result item, keys in response order."""
        return {
            "job_id": job.job_id,
            "state": job.state,
            "lease_id": job.lease_id,
