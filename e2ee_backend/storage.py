...
#: An ack-batch named a lease already acknowledged under a different
#: ack_id (an exact replay answers 200 instead). Ids are scoped to one
#: lease and may recur on other leases.
INBOX_LEASE_ACK_CONFLICT = "ack_id_conflict"
...
#: A release-batch mixed first-time items with exact replays of already
#: committed releases; the batch is atomic, so a partial replay conflicts
#: (409/items[i].lease_id of the first replayed item).
...
#: A dispatch-batch mixed first-time (pending) items with items whose
#: single-job dispatch already happened; the batch is atomic, so a partial
#: replay conflicts (409/items[i].job_id of the first replayed item).
...
#: Event types on the per-device append-only inbox redelivery-job event
#: chain (``GET /v1/devices/{device_id}/inbox-job-events``). The first
#: queue/dispatch/recover/cancel of a job each record one event under their
#: operation name; the first lease completion that moves a running job to a
#: terminal state records one ``complete`` event.
INBOX_JOB_EVENT_QUEUE = "queue"
INBOX_JOB_EVENT_DISPATCH = "dispatch"
INBOX_JOB_EVENT_RECOVER = "recover"
INBOX_JOB_EVENT_CANCEL = "cancel"
INBOX_JOB_EVENT_COMPLETE = "complete"
INBOX_JOB_EVENT_TYPES = frozenset({
    INBOX_JOB_EVENT_QUEUE,
    INBOX_JOB_EVENT_DISPATCH,
    INBOX_JOB_EVENT_RECOVER,
    INBOX_JOB_EVENT_CANCEL,
    INBOX_JOB_EVENT_COMPLETE,
})
...
class InboxLeaseCompleteBatchError(Exception):
    ...
    complete reasons (``lease_not_found`` -> 404/items[i].lease_id,
    ``lease_id_conflict`` / ``lease_unavailable`` /
    ``completion_id_conflict`` -> 409/items[i].lease_id or
...
class RedeliveryJobDispatchBatchError(Exception):
    ...
    items with items whose single-job dispatch already happened.
    """
...
class RedeliveryJobStatusBatchError(Exception):
    ...
    offending item; items are prechecked in array order and the batch is
    read-only, so nothing is ever written. Reasons are the lookup reasons
    of :class:`RedeliveryJobError` (``job_not_found`` ->
    404/items[i].job_id, ``job_id_conflict`` ->
    409/items[i].job_id).
    """
...
class DeviceStore:
    ...
    def __init__(self) -> None:
        ...
        # Append-only inbox redelivery-job event chains, keyed by
        # device_id. Each first-time job lifecycle transition appends
        # exactly one event (in batch input order inside a batch), inside
        # the same locked transaction as the job mutation, so the chain is
        # persisted and rolled back together with the rest of the state.
        self._inbox_job_events: Dict[str, List[InboxJobEvent]] = {}
    ...
    def _append_inbox_job_event_locked(
            self, job: RedeliveryJob, event_type: str) -> InboxJobEvent:
        """Append one inbox redelivery-job event to a device's chain.

        Lock required. The event's ``seq`` continues the owning device's
        chain (starting at 1); its ``state`` freezes the job's post-commit
        state (one of the five job states). Callers append only for real
        first-time lifecycle transitions — the first
        queue/dispatch/recover/cancel and the first completion that moves a
        ``running`` job to ``succeeded``/``failed``; replays, failed
        requests and ordinary lease operations append nothing. Batch
        callers invoke this in the batch's input order, so a batch's events
        land in input order (possibly interleaved with a completion's event
        within one transaction).
        """
        chain = self._inbox_job_events.setdefault(job.device_id, [])
        event = InboxJobEvent(
            device_id=job.device_id, seq=len(chain) + 1,
    ...
    def inbox_lease_release_batch(
        ...
        strings already validated for shape and uniqueness (no ``lease_id``
        repeats) by the service. Under the one store lock — shared with
        claims, the single release, renewals, completions, acks, retries,
        redelivery jobs and revocation — the device is resolved first
        ...
        a first-time release naming an already completed lease
        ``lease_unavailable`` (409/items[i].lease_id). An already-released
        lease is an exact replay: it skips the completion check and
        answers with its frozen release timestamp — expiry alone never
        blocks a first release, exactly as the single-lease entry.
        ...
        first-time lease is released in input order, the same UTC
        ``released_at`` (six microsecond digits, ``+00:00``) stamped onto
        every delivery record each lease was taken on. The whole batch
        commits with one persistence notification (201, commit_seq + 1; a
        durable write failure rolls every record back). Returns
        ``(results, status_code)`` with one three-field
        (``lease_id``/``released_at``/``released_count``) result per item,
        in input order (replays carry their frozen values);
        ``released_count`` is the number of messages the lease claimed.
        """
        with self._lock:
            device = self._find_device(device_id)
            if device is None:
            ...
            if device.revoked:
                raise InboxLeaseError(INBOX_LEASE_DEVICE_INACTIVE)
            # Resolve every lease once: first-seen is authoritative
            # (restore validation keeps the per-delivery copies identical).
            resolved: List[List[Tuple[str, str]]] = []
            replays: List[bool] = []
            ...
            for index, lease_id in enumerate(items):
                ...
                owner, _limit, _deadline, released_at, keys = existing
                # A cross-device release is a conflict regardless of the
                # path device's current state, mirroring the claim replay
                # rules.
                if owner != device_id:
                    raise InboxLeaseReleaseBatchError(
                ...
                if released_at is not None:
                    # Exact replay: the frozen first response, however
                    # stale the request; the release check is skipped just
                    # like the single-lease entry skips it.
                    replays.append(True)
                    frozen.append(released_at)
                    continue
            ...
            if all(replays):
                ...
                return [
                    {"lease_id": lease_id, "released_at": released_at,
                     "released_count": len(keys)}
                    for lease_id, released_at, keys
                    in zip(items, frozen, resolved)
                ], 200
            if any(replays):
            ...
            # Every first-time item shares the one batch release timestamp.
            released_at = datetime.now(timezone.utc) \
                .isoformat(timespec="microseconds")
            for lease_id, keys in zip(items, resolved):
                for key in keys:
                    state = self._delivery.get(key)
                    if state is None:
                        continue
    ...
    def inbox_lease_complete_batch(
        ...
        """Atomically complete many 1:1-inbox leases of one device.

        ``POST /v1/inbox-jobs/complete-batch``; *items* are
        ``(lease_id, completion_id, outcome)`` triples already validated for
        shape, outcome and per-field uniqueness (no ``lease_id`` or
        ``completion_id`` repeats) by the service. Under the one store lock
        — shared with claims, releases, renewals, the single completions,
        acks, redelivery jobs and revocation — the device is resolved first
        (unknown/revoked -> :class:`InboxLeaseError`
        ``device_unknown``/``device_inactive``, 409/device_id).

        Every item is then prechecked in input order with exactly the
        single-lease completion rules, the first failure raising
        :class:`InboxLeaseCompleteBatchError` carrying that item's index and
        writing nothing: a never-committed ``lease_id`` raises
        ``lease_not_found`` (404/items[i].lease_id), a lease owned by
        another device ``lease_id_conflict`` (409/items[i].lease_id), an
        already-completed lease whose completion_id differs or a reused
        completion id ``completion_id_conflict``
        (409/items[i].completion_id), and an unfinished but released or
        expired lease ``lease_unavailable`` (409/items[i].lease_id). An item
        replaying its own committed completion (same id and outcome) skips
        those checks, as the single-lease entry does.

        Only after every item passes does the batch resolve: when every
        item is such a replay the frozen first responses are returned with
        200 and nothing is written; a mix of replays and first-time items
        cannot be applied atomically and raises
        ``complete_partial_replay`` for the first replayed item
        (409/items[i].completion_id). Otherwise every first-time lease is
        completed in input order with one shared UTC timestamp (six
        microsecond digits, ``+00:00``) and the whole batch commits with one
        persistence notification (201, commit_seq + 1; a durable write
        failure rolls every lease and redelivery job back); each item's
        associated running redelivery job moves to ``succeeded``/``failed``
        with its outcome in the same locked transaction. Returns
        ``(results, status_code)`` with one four-field
        (``lease_id``/``completion_id``/``outcome``/``completed_at``) result
        per item, in input order (replays carry their frozen values).
        """
        with self._lock:
            now = datetime.now(timezone.utc)
            device = self._find_device(device_id)
            ...
            resolved: List[List[Tuple[Tuple[str, str], MessageLease]]] = []
            replays: List[bool] = []
            for index, (lease_id, completion_id, outcome) in enumerate(items):
                hits: List[Tuple[Tuple[str, str], MessageLease]] = []
                for key, state in self._delivery.items():
                    for lease in state.leases:
                        if lease.lease_id == lease_id:
                ...
                existing = hits[0][1].completion
                if existing is not None:
                    # The replay decision precedes every other state check,
                    # mirroring the single-lease completion: an exact replay
                    # of the same completion_id and outcome answers the
                ...
                if any(not self._lease_is_active_locked(lease, now)
                    ...
                    raise InboxLeaseCompleteBatchError(
                        INBOX_LEASE_UNAVAILABLE, index)
            if all(replays):
                # Every item replays its committed completion: answer the
                # frozen responses with 200 and write nothing (no
                # persistence notification, no commit_seq advance).
                return [
                    self._complete_batch_item_view(
                        lease_id, hits[0][1].completion)
                    for (lease_id, _cid, _outcome), hits
                    in zip(items, resolved)
                ], 200
            if any(replays):
                # A partial replay cannot commit atomically: the batch is
                # all-or-nothing, so the first replayed item conflicts.
                raise InboxLeaseCompleteBatchError(
                    INBOX_LEASE_COMPLETE_PARTIAL_REPLAY, replays.index(True))
            ...
            completed_at = now.isoformat(timespec="microseconds")
            for (lease_id, completion_id, outcome), hits in zip(
                    items, resolved):
                completion = MessageLeaseCompletion(
                    completion_id=completion_id, outcome=outcome,
                    completed_at=completed_at)
                for _key, lease in hits:
                    lease.completion = completion
                # A lease taken by a redelivery job moves its job to the
                # matching terminal state in the same locked transaction.
                self._redelivery_job_completed_locked(lease_id, outcome)
            # One persistence notification for the whole batch: every lease
            # completion (and redelivery-job transition) commits or rolls
            # back together and the generation advances exactly once.
            self._notify_change()
            return [
                {"lease_id": lease_id, "completion_id": completion_id,
                 "outcome": outcome, "completed_at": completed_at}
                for lease_id, completion_id, outcome in items
            ], 201

    @staticmethod
    def _complete_batch_item_view(
            lease_id: str,
            completion: MessageLeaseCompletion) -> Dict[str, Any]:
        """One complete-batch replay result item, keys in response order."""
        return {
            "lease_id": lease_id,
    ...
    def inbox_lease_ack_batch(
        ...
        """Atomically bulk-acknowledge many delivered 1:1-inbox leases.

        ``POST /v1/inbox-jobs/ack-batch``; *items* are ``(lease_id,
        ack_id)`` pairs already validated for shape and per-field
        uniqueness (no ``lease_id`` or ``ack_id`` repeats) by the service.
        Under the one store lock — shared with claims, releases, renewals,
        completions, the single and batched acks, retries, redelivery jobs
        and revocation — the device is resolved first (unknown/revoked ->
        :class:`InboxLeaseError` ``device_unknown``/``device_inactive``,
        409/device_id).

        Every item is then prechecked in input order, the first failure
        raising :class:`InboxLeaseAckBatchError` carrying that item's index
        and writing nothing: a never-committed ``lease_id`` raises
        ``lease_not_found`` (404/items[i].lease_id), a lease owned by
        another device ``lease_id_conflict`` (409/items[i].lease_id), an
        already-acknowledged lease whose recorded ack_id differs
        ``ack_id_conflict`` (409/items[i].ack_id), and a lease without a
        ``delivered`` completion ``lease_not_delivered``
        (409/items[i].lease_id). An item replaying its own committed ack
        (same ack_id) skips the remaining checks.

        Only after every item passes does the batch resolve: when every
        item is such a replay the frozen first responses are returned with
        200 and nothing is written (no persistence notification, no
        commit_seq advance); a mix of replays and first-time items cannot
        be applied atomically and raises ``ack_partial_replay`` for the
        first replayed item (409/items[i].ack_id). Otherwise every
        first-time lease is acknowledged in input order: each delivery
        record the lease lives on is set ``acked=True`` with
        ``ack_sequence`` equal to the message's own sequence (``attempts``
        and the attempt-id dedup set untouched) and the lease freezes its
        ``ack_id`` on every copy. The whole batch commits with one
        persistence notification (201, commit_seq + 1; a durable write
        failure rolls every record back). Returns ``(results,
        status_code)`` with one three-field
        ...
        with self._lock:
            ...
            for index, (lease_id, ack_id) in enumerate(items):
                ...
                if owner != device_id:
                    raise InboxLeaseAckBatchError(
                        INBOX_LEASE_CONFLICT, index)
                resolved.append(hits)
                recorded = hits[0][1].ack_id
                if recorded is not None:
                    # The replay decision precedes the delivery-state
                    # check: an exact replay of the same ack_id answers the
                    # frozen first response, however stale the request.
                    if recorded == ack_id:
                        replays.append(True)
                        continue
                    raise InboxLeaseAckBatchError(
                        INBOX_LEASE_ACK_CONFLICT, index)
                replays.append(False)
                completion = hits[0][1].completion
                if completion is None or completion.outcome != "delivered":
                    raise InboxLeaseAckBatchError(
            ...
            if all(replays):
                # Every item replays its committed acknowledgement: answer
                # the frozen responses with 200 and write nothing.
                return [
            ...
            for (lease_id, ack_id), hits in zip(items, resolved):
                for (session_id, message_id), lease in hits:
                    state = self._delivery[(session_id, message_id)]
                    sequence = next(
                        m.sequence for m in self._messages[session_id]
                        if m.message_id == message_id)
                    state.acked = True
                    state.ack_sequence = sequence
                    lease.ack_id = ack_id
    ...
    @staticmethod
    def inbox_job_event_view(event: InboxJobEvent) -> Dict[str, Any]:
        """The wire view of one inbox-job event, keys in response order."""
        return {
            "seq": event.seq,
            "job_id": event.job_id,
            "type": event.type,
    ...
    def inbox_job_events_page(self, device_id: str, after: int, limit: int
                              ) -> Optional[Dict[str, Any]]:
        """Read one ascending page of a device's inbox-job event chain.

        ``GET /v1/devices/{device_id}/inbox-job-events``. Under the one
        store lock — shared with every job/lease mutation that appends an
        event — an unknown *device_id* returns ``None`` (404/device_id at
        the service layer); a revoked device's chain stays readable. The
        page carries the events with ``seq > after`` (ascending, at most
        *limit*), ``next_after`` — the last returned ``seq``, or *after*
        itself when the page is empty — and ``has_more``. The lookup and
        page copy run under the lock, so a concurrent append is linearized
        either wholly before or wholly after this read; it writes nothing
        and advances no commit generation.
        """
        with self._lock:
            device = self._find_device(device_id)
            if device is None:
                return None
            chain = self._inbox_job_events.get(device_id, [])
            page = [event for event in chain if event.seq > after][:limit]
            next_after = page[-1].seq if page else after
            has_more = any(event.seq > next_after for event in chain)
            return {
                "device_id": device_id,
                "events": [self.inbox_job_event_view(event)
                           for event in page],
                "next_after": next_after,
                "has_more": has_more,
            }
    ...
    @staticmethod
    def redelivery_job_status_view(job: RedeliveryJob) -> Dict[str, Any]:
        """The wire view of one batch job-status item, keys in response order.

        Like :meth:`redelivery_job_detail_view` but without the repeated
        ``device_id`` (the batch body carries it once): ``job_id``,
        ``state``, ``lease_id``, ``recoveries``, ``cancellation_id`` and
        ``cancelled_at``.
        """
        return {
            "job_id": job.job_id,
    ...
    def redelivery_job_status_batch(
            self, device_id: str,
            job_ids: List[str]
    ) -> List[Dict[str, Any]]:
        """Read many 1:1-inbox redelivery jobs' current state (read-only).

        ``POST /v1/inbox-jobs/status-batch``; *job_ids* are non-empty
        strings already validated for shape and uniqueness (no repeats) by
        the service. Under the one store lock — shared with the job
        queue/dispatch/recover/cancel operations, lease
        claims/renewals/releases/completions, acks and revocation — the
        device is resolved first: an unknown *device_id* raises
        :class:`RedeliveryJobError` ``device_unknown``
        (404/device_id). Unlike the mutating and lease batch routes, a
        revoked device stays queryable, exactly as for the single-job GET.

        Every item is then prechecked in input order, the first failure
        raising :class:`RedeliveryJobStatusBatchError` carrying that item's
        index: a never-queued ``job_id`` raises ``job_not_found``
        (404/items[i].job_id) and an id committed for another device raises
        ``job_id_conflict`` (409/items[i].job_id). The precheck runs ahead
        of building any result, so the first error fails the whole batch.

        Returns one result per item, in input order, each the
        :meth:`redelivery_job_status_view` of the job — ``state`` one of
        the five job states, ``lease_id`` and the two cancellation fields a
        non-empty string or ``None``, ``cancelled_at`` the persisted UTC
        stamp, and ``recoveries`` the recovery chain in commit order (each
        item ``recovery_id`` then ``lease_id``). The query writes nothing
        ...
        with self._lock:
            ...
            if device is None:
                raise RedeliveryJobError(REDELIVERY_JOB_DEVICE_UNKNOWN)
            # Precheck every item in input order; the first failure aborts
            # the whole (read-only) batch.
            ...
            for index, job_id in enumerate(job_ids):
                ...
                jobs.append(job)
            return [self.redelivery_job_status_view(job) for job in jobs]
    ...
    def redelivery_job_submit(
        ...
        with self._lock:
            ...
            if op == "queue":
                ...
                self._append_inbox_job_event_locked(
                    job, INBOX_JOB_EVENT_QUEUE)
                self._notify_change()
            ...
            self._append_inbox_job_event_locked(
                job, INBOX_JOB_EVENT_DISPATCH)
            self._notify_change()
            return self.redelivery_job_view(job), 201

    def _redelivery_job_apply_dispatch_locked(
            self, job: RedeliveryJob, now: datetime) -> None:
        """Dispatch one pending *job*: lease its messages or end it succeeded.

        Lock required; the caller guarantees the job is ``pending`` and its
        ...
        still-active lease at *now*. A non-empty selection is leased for the
        usual :data:`INBOX_LEASE_SECONDS` window under a lease whose
        ``lease_id`` is the ``job_id`` itself (recorded on every leased
        message's delivery record, creating one if absent): the job moves to
        ``running`` with ``lease_id`` set. An empty selection moves it
        straight to ``succeeded`` with ``lease_id`` null and never takes a
        lease. Shared by the single dispatch and the atomic dispatch batch;
        the caller is responsible for the persistence notification, so a
        batch dispatching many jobs notifies exactly once.
        """
        picked: List[Tuple[str, str]] = []
        for _created_at, session_id, message in self._inbox_entries_locked(
                job.device_id):
            key = (session_id, message.message_id)
            state = self._delivery.get(key)
            if state is not None \
        ...
        if picked:
            # timespec="microseconds" always emits six fractional digits,
            # mirroring the claim endpoint's deadlines. One shared *now*
            # across a batch therefore yields one identical deadline.
            ...
            # Nothing to redeliver: the job succeeds immediately and never
            # takes a lease.
            job.state = REDELIVERY_JOB_SUCCEEDED
            job.lease_id = None

    def redelivery_job_dispatch_batch(
            self, device_id: str, job_ids: List[str]
    ) -> Tuple[List[Dict[str, Any]], int]:
        """Atomically dispatch many pending redelivery jobs of one device.

        ``POST /v1/inbox-jobs/dispatch-batch``; *job_ids* are already
        validated as non-empty, unique strings by the service. Under the one
        store lock — shared with the single job operations, leases,
        completions, acks and revocation — the device is resolved first
        (unknown/revoked -> :class:`RedeliveryJobError`
        ``device_unknown``/``device_inactive``, 409/device_id).
        ...
        ``job_not_found`` (404/items[i].job_id) and a job of another device,
        or one whose id is already occupied as an inbox lease id, raises
        ``job_id_conflict``/``lease_occupied`` (409/items[i].job_id). A job
        no longer ``pending`` is an item whose dispatch already happened and
        is only classified here.

        Only after every item passes does the batch resolve: when every item
        is such a replay the jobs' current views are returned with 200 and
        nothing is written; a mix of replays and first-time (pending) items
        cannot be applied atomically and raises ``dispatch_partial_replay``
        for the first replayed item (409/items[i].job_id). Otherwise each
        pending job is dispatched in input order via
        :meth:`_redelivery_job_apply_dispatch_locked` — each takes the
        inbox's next up to 100 unacked messages without a still-active lease
        under a 30-second lease named by its own ``job_id``; an earlier
        item's fresh lease already withholds its messages from later items,
        so a non-empty pick moves the job to ``running`` and an empty pick
        moves it to ``succeeded`` with ``lease_id`` null — and the whole
        batch commits with one persistence notification (201, commit_seq + 1;
        a durable write failure rolls every job and lease back). Returns
        ...
        with self._lock:
            ...
            for index, job_id in enumerate(job_ids):
                ...
                if job.state != REDELIVERY_JOB_PENDING:
                    # A dispatch already happened on this job: the single
                    # op=dispatch entry would replay its current view (200)
                    # and write nothing.
                    first_time.append(False)
                    continue
                # The dispatch lease takes the job_id as its lease_id; an
                # unrelated claim that already occupies the id blocks the
                # dispatch rather than corrupting the lease namespace. The
                # batch's own job_ids are unique, so no earlier item of this
                # batch can occupy a later item's id.
                if self._find_inbox_lease_locked(job_id) is not None:
                    raise RedeliveryJobDispatchBatchError(
                        REDELIVERY_JOB_LEASE_OCCUPIED, index)
                first_time.append(True)
            if not any(first_time):
                # Every item replays an already-applied dispatch: answer the
                # current views with 200 and write nothing (no persistence
                # notification, no commit_seq advance).
                return [self._recover_batch_item_view(job) for job in jobs], \
                    200
            if not all(first_time):
                # A partial replay cannot commit atomically: the batch is
                # all-or-nothing, so the first replayed item conflicts.
                raise RedeliveryJobDispatchBatchError(
                    REDELIVERY_JOB_DISPATCH_PARTIAL_REPLAY,
                    first_time.index(False))
            # Dispatch in input order: each item's fresh lease immediately
            # withholds its messages from the items that follow. The batch
            # only commits when every item is a first-time dispatch, so each
            # one records its dispatch event here, in input order.
            for job in jobs:
                self._redelivery_job_apply_dispatch_locked(job, now)
                self._append_inbox_job_event_locked(
                    job, INBOX_JOB_EVENT_DISPATCH)
        ...
        *outcome*. A dispatch lease's id is the job_id itself, but a lease
        ...
        for job in self._redelivery_jobs.values():
            if job.state == REDELIVERY_JOB_RUNNING \
                ...
                # The first completion of the current lease ends the job;
                # record its terminal event in this same transaction. Only a
                # still-running job can match, so exactly one event is ever
                # appended per job here. A completion that moves no job is an
                # ordinary lease operation and records nothing.
                self._append_inbox_job_event_locked(
                    job, INBOX_JOB_EVENT_COMPLETE)
                return
    ...
    def snapshot_state(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Flat list over the per-device chains; each item repeats
            # device_id first so the chains can be reconstructed on load.
            # Chains stay append-only in seq order, so iterating the chains
            # yields the restorable order (a device's events contiguous).
            redelivery_job_events = [{
                "device_id": event.device_id,
                "seq": event.seq,
                "job_id": event.job_id,
                "type": event.type,
                "state": event.state,
            } for chain in self._inbox_job_events.values()
                for event in chain]
            key_events = [
    ...
    def restore_state(self, state: Dict[str, Any]) -> None:
        ...
        # release timestamp, one renewal list, one completion and one
        # ack_id across every delivery record it appears on; a
        ...
        for index, raw in enumerate(raw_delivery):
                ...
                for l_index, raw_lease in enumerate(raw_leases):
                    ...
                    if completion is not None and released_at is not None:
                        raise ValueError(
                            ...
                            f"completion")
                    # The bulk-ack id. Older files predate the key: absent
                    # means null (never acked through the batch entry). A
                    # present value must be null or a non-empty string; a
                    # non-null id is only possible on a lease completed
                    # ``delivered`` whose every carrying delivery record is
                    # acked with the message's own sequence (checked below,
                    # once the record's ack cursor is known).
                    ack_id = raw_lease.get("ack_id")
                    if ack_id is not None \
                            and not (isinstance(ack_id, str) and ack_id):
                        raise ValueError(
                            f"{l_where}.ack_id must be null or a non-empty "
                            f"string")
                    if ack_id is not None and (
                            completion is None
                            or completion.outcome != "delivered"):
                        raise ValueError(
                            f"{l_where}.ack_id requires a 'delivered' "
                            f"completion")
            ...
            # A lease acked through the batch entry carries its ack_id on
            # every delivery record it lives on, and each of those records
            # is itself acked at the message's own sequence.
            if any(lease.ack_id is not None for lease in leases) \
                    and not acked:
                raise ValueError(
                    f"{where} must be acked while carrying an "
                    f"acknowledged lease")
        ...
        # Per-device inbox redelivery-job event chains. Older version-1
        # files predate the section: it is absent and treated as empty (a
        # job with no events is fully compatible). A present section is a
        # flat list whose every item carries exactly the five keys
        # device_id/seq/job_id/type/state in that order: non-empty device
        # and job ids referencing a registered device and its own job, a
        # positive non-boolean seq, one of the five event types and one of
        # the five job states. The per-device seqs must run consecutively
        # from 1, each event's state must match the transition its type
        # makes, and each job's last event state must equal that job's
        # current state in the redelivery_jobs section. Any contradiction
        # refuses startup and leaves the file untouched.
        inbox_job_events: Dict[str, List[InboxJobEvent]] = {}
        for index, raw in enumerate(raw_redelivery_job_events):
            ...
            if not (isinstance(e_job_id, str) and e_job_id):
                raise ValueError(
                    f"{where}.job_id must be a non-empty string")
            if e_type not in INBOX_JOB_EVENT_TYPES:
                raise ValueError(
                    f"{where}.type must be one of "
                    f"{sorted(INBOX_JOB_EVENT_TYPES)}")
            ...
            target_job = redelivery_jobs.get(e_job_id)
            if target_job is None:
                raise ValueError(
                    f"{where} references an unknown redelivery job: "
                    f"{e_job_id}")
            if target_job.device_id != e_device_id:
                raise ValueError(
                    f"{where} references a job owned by another device: "
                    f"{e_job_id}")
            # The state frozen on an event is the job's state right after
            # that operation committed, so each type only allows the states
            # its transition can produce.
            allowed_states = {
                INBOX_JOB_EVENT_QUEUE: {REDELIVERY_JOB_PENDING},
                INBOX_JOB_EVENT_DISPATCH: {
                    REDELIVERY_JOB_RUNNING, REDELIVERY_JOB_SUCCEEDED},
                INBOX_JOB_EVENT_RECOVER: {
                    REDELIVERY_JOB_RUNNING, REDELIVERY_JOB_SUCCEEDED},
                INBOX_JOB_EVENT_CANCEL: {REDELIVERY_JOB_CANCELLED},
                INBOX_JOB_EVENT_COMPLETE: {
                    REDELIVERY_JOB_SUCCEEDED, REDELIVERY_JOB_FAILED},
            }[e_type]
            if e_state not in allowed_states:
                raise ValueError(
                    f"{where}.state {e_state} is not a state a {e_type} "
                    f"event can produce")
            inbox_job_events.setdefault(e_device_id, []).append(
                InboxJobEvent(device_id=e_device_id, seq=e_seq,
                              job_id=e_job_id, type=e_type, state=e_state))
        # Per-device seqs run consecutively from 1; the section is flat
        # across devices, so the chains are regrouped and seq-sorted first.
        events_by_job_last: Dict[str, str] = {}
        for event_device_id, chain in inbox_job_events.items():
            chain.sort(key=lambda event: event.seq)
            for position, event in enumerate(chain):
                if event.seq != position + 1:
                    raise ValueError(
                        f"redelivery_job_events chain of device "
                        f"{event_device_id} must run consecutively from seq 1")
                events_by_job_last[event.job_id] = event.state
        # Each job carrying events must end (at its highest-seq event) on
        # its current state; a job with no events is tolerated (older files
        # predate the section).
        for e_job_id, last_state in events_by_job_last.items():
            if redelivery_jobs[e_job_id].state != last_state:
                raise ValueError(
                    f"redelivery job {e_job_id} last event state "
                    f"{last_state} does not match its current state "
                    f"{redelivery_jobs[e_job_id].state}")
        ...
        with self._lock:
            ...
            self._inbox_job_events = inbox_job_events
