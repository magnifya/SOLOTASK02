...
class DeviceService:
    ...
    def inbox_job_lease_status_batch(
            self, payload: object) -> Dict[str, Any]:
        """Validate and answer a read-only batch lease-status query.

        ``POST /v1/inbox-jobs/lease-status-batch``. The body must be a JSON
        object carrying a non-empty string ``device_id`` and a non-empty
        ``items`` array and no other top-level key; each item is an object
        carrying exactly the non-empty string ``lease_id``, and no
        ``lease_id`` may repeat across items. Shape errors are reported, in
        order, as 400/field ``request_body`` (bad/non-object body),
        ``device_id`` (missing/empty/non-string), ``items``
        (missing/not-a-non-empty array), the first extra top-level key,
        ``items[i]`` (non-object element, an unexpected key, or a repeated
        ``lease_id``) or ``items[i].lease_id`` for the offending field.

        The device is then resolved (unknown/revoked -> 409/field
        ``device_id``) and every item is prechecked in array order, the
        first error aborting the whole query with its ``field`` prefixed to
        ``items[i].``: an unknown lease is 404/``items[i].lease_id`` and a
        lease owned by another device is 409/``items[i].lease_id``. On
        success the body keys are ``device_id`` then ``results``; results
        keep input order and each item is ``lease_id``, ``state``,
        ``leased_until``, ``released_at``, ``completion`` and
        ``message_count`` in that order. Every lease in the batch is judged
        at the same locked instant — completion wins over release, which
        wins over expiry — and the query writes nothing, persists nothing
