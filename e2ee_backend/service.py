...
class DeviceService:
    ...
    def inbox_jobs(self, device_id: str, state: str, after: int,
                   limit: int) -> Dict[str, Any]:
        ...
        Under the store lock the jobs are kept in first-successful-``queue``
        commit order, filtered to the path device's own jobs and the
        requested current state, then paged (skip *after*, take *limit*).
        An unknown device is 404/field ``device_id``; a revoked device's
        history stays readable. On success the body keys are
        ``device_id``, ``jobs``, ``next_after`` and ``has_more`` in that
        order; each job item is ``job_id``, ``state``, ``lease_id``,
        ``cancellation_id`` and ``cancelled_at`` (the last three strings
        or null). The lookup writes nothing and advances no
        ``commit_seq``.
        """
        ...
        if not isinstance(limit, int) or isinstance(limit, bool) \
            ...
            raise ServiceError(
                "field must be an integer in 1..100: limit", "limit")
        page = self.store.inbox_jobs_page(device_id, state, after, limit)
        if page is None:
            raise ServiceError(f"device not found: {device_id}",
                               "device_id", status_code=404)
        return page
