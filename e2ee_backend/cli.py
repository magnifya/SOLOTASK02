...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_create_session_claim = sub.add_parser(
        "create-session-from-claim",
        help="establish the one session bound to a pre-key claim")
    p_create_session_claim.add_argument("--claim-id", required=True)
    p_create_session_claim.add_argument("--initiator-device-id", required=True)
    p_create_session_claim.add_argument(
        "--ephemeral-key", required=True,
        help="ephemeral public key (string), or @path to read it from a file")
    p_create_session_claim.set_defaults(handler=cmd_create_session_from_claim)
...
def cmd_create_session_from_claim(args: argparse.Namespace) -> int:
    """Call POST /v1/sessions/from-claim and print the single-line JSON."""
    payload = {
        "claim_id": args.claim_id,
        "initiator_device_id": args.initiator_device_id,
        "ephemeral_key": _read_key_argument(args.ephemeral_key),
