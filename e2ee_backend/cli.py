...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_sync_group = sub.add_parser(
        "sync-group-messages",
        help="sync a page of group-session messages from a device's cursor")
    p_sync_group.add_argument("session_id")
    p_sync_group.add_argument("--device-id", required=True)
    p_sync_group.add_argument(
        "--after", type=int, default=None,
        help="read after this sequence; omitted to resume from the device's "
             "stored cursor (and advance it)")
    p_sync_group.add_argument("--limit", type=int, default=100)
    p_sync_group.set_defaults(handler=cmd_sync_group_messages)

    p_sync_checkpoint = sub.add_parser(
        "sync-checkpoint",
        help="set a device's group-session sync checkpoint cursor")
    p_sync_checkpoint.add_argument("session_id")
    p_sync_checkpoint.add_argument("--device-id", required=True)
    p_sync_checkpoint.add_argument(
        "--cursor", required=True, type=int,
        help="new cursor (0..largest message sequence); forward moves 201, "
             "unchanged 200, backwards 409")
...
def cmd_sync_group_messages(args: argparse.Namespace) -> int:
    ...
    ``--after`` is omitted from the query when not supplied, which tells the
    server to resume from (and advance) the device's stored cursor.
    """
    from urllib.parse import quote, urlencode

    params: List[Tuple[str, object]] = [
        ("device_id", args.device_id), ("limit", args.limit)]
