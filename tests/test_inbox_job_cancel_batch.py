...
An atomic batch cancellation of 1:1-inbox redelivery jobs: the body
carries a ``device_id`` and a non-empty ``items`` array of
``job_id``/``cancellation_id`` pairs (both unique across items). Every
item is prechecked in array order with the single-job ``op=cancel``
rules (first error aborts the batch, its ``field`` prefixed to
``items[i].``); only then does the batch cancel in input order and
commit once. A batch whose items all replay their committed
cancellations answers 200 and writes nothing; a partial replay
...
class _BatchMixin(InboxMixin):
    ...
    def _pending_and_running_jobs(self):
        # jp stays pending; jr dispatches and leases the whole inbox as a
        # running job.
        self._job(job_id="jp")
        self._dispatch(job_id="jr")
...
class InboxJobCancelBatchServiceTest(_BatchMixin, unittest.TestCase):
    ...
    def test_extra_top_level_key_rejected_with_that_field(self) -> None:
        error = self._error(lambda: self._batch(
            items=[{"job_id": "jp", "cancellation_id": "c1"}],
            op="cancel"))
    ...
    def test_first_batch_cancels_pending_and_running_in_input_order(
            self) -> None:
        self._pending_and_running_jobs()
        body, status = self._batch(items=[
            {"job_id": "jp", "cancellation_id": "cp"},
            {"job_id": "jr", "cancellation_id": "cr"}])
        self.assertEqual(status, 201)
        self.assertEqual(list(body), ["device_id", "results"])
        self.assertEqual(body["device_id"], "bob")
        self.assertEqual(body["results"], [
            {"job_id": "jp", "state": "cancelled", "lease_id": None},
            {"job_id": "jr", "state": "cancelled", "lease_id": "jr"},
        ])
        for item in body["results"]:
            self.assertEqual(list(item), ["job_id", "state", "lease_id"])
        # The running job's dispatch lease was released in the same
        # transaction, so the inbox is immediately claimable again.
        claim, status = self.service.inbox_claim(
            "bob", {"lease_id": "L-after", "limit": 100})
        self.assertEqual(status, 201)
        self.assertEqual([m["message_id"] for m in claim["messages"]],
                         ["a1", "a2", "a3", "b1", "b2"])
        # The released dispatch lease is queryable as released history.
        lease = self.service.inbox_lease_get("bob", "jr")
    ...
    def test_per_item_errors_use_indexed_fields(self) -> None:
        ...
        self.assertEqual((error.status_code, error.field),
                         (409, "items[0].job_id"))
        # A failed terminal job cannot be cancelled.
        self.service.inbox_lease_complete(
            "bob", "jr", {"completion_id": "done", "outcome": "failed"})
        error = self._error(lambda: self._batch(items=[
            {"job_id": "jr", "cancellation_id": "cr"}]))
        self.assertEqual((error.status_code, error.field),
                         (409, "items[0].job_id"))
        # A cancelled job under a different cancellation_id conflicts.
        self._batch(items=[{"job_id": "jp", "cancellation_id": "cp"}])
        error = self._error(lambda: self._batch(items=[
            {"job_id": "jp", "cancellation_id": "other"}]))
        self.assertEqual((error.status_code, error.field),
                         (409, "items[0].cancellation_id"))

    def test_terminal_jobs_conflict_at_job_id(self) -> None:
        # Dispatch a job that leases the whole inbox, fail its lease, and
        # try to cancel the now-failed job in a batch.
    ...
    def test_first_error_in_array_order_wins_and_nothing_is_written(
        ...
        # 0's cancellation is not applied.
        error = self._error(lambda: self._batch(items=[
            {"job_id": "jp", "cancellation_id": "cp"},
            {"job_id": "ghost", "cancellation_id": "c2"}]))
        self.assertEqual((error.status_code, error.field),
                         (404, "items[1].job_id"))
        body, _ = self._job(job_id="jp", op="status")
        self.assertEqual((body["state"], body["lease_id"]),
                         ("pending", None))
        # An earlier bad item wins over a later one.
        error = self._error(lambda: self._batch(items=[
            {"job_id": "ghost", "cancellation_id": "c1"},
            {"job_id": "jr", "cancellation_id": "c2"}]))
        self.assertEqual((error.status_code, error.field),
                         (404, "items[0].job_id"))
        body, _ = self._job(job_id="jr", op="status")
        self.assertEqual((body["state"], body["lease_id"]),
                         ("running", "jr"))
    ...
    def test_all_replays_answer_200_without_writing(self) -> None:
        ...
        # A single-item replay also answers 200, even after the device is
        # revoked (the device gate still runs first, though).
        replay, status = self._batch(items=[
            {"job_id": "jp", "cancellation_id": "cp"}])
        self.assertEqual(status, 200)
    ...
    def test_running_cancel_with_already_released_lease_keeps_timestamp(
            self) -> None:
        self._dispatch(job_id="jr")
        # Release the dispatch lease explicitly first.
        release, release_status = self.service.inbox_release("bob", "jr")
        self.assertEqual(release_status, 201)
        ...
        lease = self.service.inbox_lease_get("bob", "jr")
        self.assertEqual(lease["state"], "released")
        self.assertEqual(lease["released_at"], released_at)
...
class InboxJobCancelBatchPersistenceTest(_BatchMixin, unittest.TestCase):
    ...
    def test_failed_batch_writes_nothing(self) -> None:
        ...
        self.assertIsNone(jobs["jp"]["cancellation_id"])
        self.assertIsNone(jobs["jp"]["cancelled_at"])
    ...
    def test_state_file_shape(self) -> None:
        ...
        self.assertEqual(items["jp"]["state"], "cancelled")
        self.assertIsNone(items["jp"]["lease_id"])
        self.assertEqual(items["jp"]["cancellation_id"], "cp")
        self.assertEqual(items["jr"]["state"], "cancelled")
        self.assertEqual(items["jr"]["lease_id"], "jr")
        self.assertEqual(items["jr"]["cancellation_id"], "cr")
        for item in items.values():
    ...
    def test_restart_restores_batch_and_replays(self) -> None:
        ...
        body, _ = restarted.inbox_job(
            {"device_id": "bob", "job_id": "jr", "op": "status"})
        self.assertEqual((body["state"], body["lease_id"]),
                         ("cancelled", "jr"))
        # The released dispatch lease survived the restart.
        lease = restarted.inbox_lease_get("bob", "jr")
        self.assertEqual(lease["state"], "released")
...
class InboxJobCancelBatchHTTPTest(_BatchMixin, unittest.TestCase):
    ...
    def test_errors_over_http(self) -> None:
        ...
        # Partial replay: cancel jp, then mix it with a fresh item.
        ...
        self.assertEqual(status, 409)
        self.assertEqual(body["field"], "items[0].cancellation_id")
        # The failed batch wrote nothing: jr is still running.
        status, body, _ = self._request(
            {"device_id": "bob", "items": [
                {"job_id": "jr", "cancellation_id": "cr"}]})
        self.assertEqual(status, 201)
