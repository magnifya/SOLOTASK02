...
多 serve 进程并发协调：仅启用 --data-file 或 E2EE_DATA_FILE 时生效，内存模式行为不变。serve 启动时（在任何状态文件读取、崩溃遗留恢复或创建之前）先在状态文件同目录建立进程级排他锁文件（.<状态文件名>.lock，以非阻塞 flock(LOCK_EX) 持有，O_CREAT 但绝不 O_TRUNC，正式状态文件不被覆盖或截断）；锁已被另一存活进程持有时拒绝启动，stderr 输出单行 JSON 且 field=data_file，退出码 1。锁文件命名不与 .state-*.tmp/.state-*.bak 崩溃遗留模式冲突，崩溃遗留扫描忽略它；锁文件本身在磁盘上保留但不再代表锁定。锁持有期间该进程的设备、群组、消息、投递、同步游标、检查点与撤销写入仍沿用既有临时文件、文件 fsync、硬链接备份、os.replace 原子替换及父目录 fsync 的事务与回滚语义（任一步失败 503/field=data_file、内存回滚、旧文件字节与 inode 不变、清理临时遗留）。首个进程正常退出或被异常终止（含 SIGKILL）后，操作系统随进程关闭文件描述符自动释放 flock，无需删除锁文件；后续进程即可获得锁启动，并仍先执行既有 version=1 结构校验、跨实体语义校验、崩溃遗留处理与旧字段兼容规则，然后完整恢复 cursor、updated_at、消息、nonce、delivery 与撤销状态。HTTP 路由、状态码、响应字段、CLI 子命令及成功 stdout/失败 stderr 契约均不改变。
