...
- `POST /v1/devices/{device_id}/inbox/claim`：为 **1:1** 收件箱补投领取 30 秒租约。请求体须为对象且含非空字符串 `lease_id` 与非布尔整数 `limit`（1..100）：坏 JSON/体非对象 `400/request_body`，`lease_id` 缺失/空/非字符串 `400/lease_id`，`limit` 缺失/非整数/布尔/越界 `400/limit`；路径设备未知或已撤销 `409/field=device_id`。锁内按收件箱顺序 `(session.created_at, session_id, sequence)` 选取至多 `limit` 条未确认且当前无有效租约（无租约或租约均已过 30 秒期限）的消息，统一钉到 30 秒后的同一截止时间；非空 `201`，空集 `200`、`leased_until` 为 `null` 且不写盘、不占 `lease_id`、不增 `commit_seq`。响应键序 `device_id`、`lease_id`、`leased_until`、`messages`；非空 `leased_until` 为 UTC ISO-8601（6 位微秒、`+00:00`），`messages` 各项沿用收件箱七键顺序。已占用 `lease_id` 同设备同 `limit` 重放恒 `200` 并返回与首次完全相同的响应（即使此后设备撤销或租约过期）；跨设备或改 `limit` 一律 `409/field=lease_id`（路径设备自身未知/撤销仍先报 `409/device_id`）。过期租约不再阻挡新 `lease_id` 领取，但确认后的消息不再入选；确认、领取、设备撤销共用存储同一把锁线性化。非空领取单次落盘、`commit_seq` 恰加 1；落盘失败 `503/field=data_file` 并回滚内存租约、代次与旁车。`version=1` 不新增顶层键：`delivery` 每项在原六键之后追加 `leases` 数组（旧文件缺该键按空加载），元素键序 `lease_id`、`limit`、`leased_until`；恢复时校验类型（`lease_id` 非空字符串、`limit` 为 1..100 非布尔整数、`leased_until` 为带 `+00:00` 的 ISO-8601）、同一记录内 `lease_id` 不重复、同一 `lease_id` 跨设备复用或 `limit`/`leased_until` 不一致，以及租约只钉 1:1 会话消息；任一矛盾拒绝启动且不覆盖原文件。无 CLI 子命令；既有 inbox、retry-batch、ack 与其余入口不变。
