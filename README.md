...
启动恢复在结构合法之外再做跨实体一致性校验，禁止语义矛盾的 version=1 状态被加载。sessions 的 initiator_device_id、recipient_device_id 必须指向已注册设备（历史快照不因设备事后撤销而失效），prekey_id 必须属于 recipient 自身的预钥集合（含已撤销钥）；session_id 不得重复，快照 identity_key/public_key、临时 ephemeral_key 与 created_at 等全部字符串字段必须是非空字符串，布尔/数字/null/空串一律拒绝。groups 的 creator 必须是已注册设备且冻结在 members 首位，members 必须非空、元素均为非空字符串且无重复，revision 必须是正整数（拒绝布尔、浮点、字符串、0 与负数），group_id 不得重复。group_sessions 必须关联已存在群组，session_id 不得重复，members 为非空、去重、元素非空字符串的冻结快照，initiator 必须属于该 members，revision 为正整数且不得超过所属群组当前 revision。devices 的全部标识与字符串字段（含 prekey 的 key_id/public_key）拒绝布尔、数字、null 等错误类型，设备内 key_id 不得重复；rotated_at 与 revoked 等字段缺失的旧 version=1 文件仍按既有缺省规则加载。messages、delivery、used_nonces、group_sync_cursors 的既有引用与一致性校验继续生效。任一悬空引用、重复标识或成员、类型错误、修订越界或字段缺失都使启动拒绝：stderr 输出单行 JSON 且 field=data_file、退出码非零，原文件字节与 inode 保持不变（不发生原子替换）。恢复成功后 HTTP、CLI、同步游标、撤销、重放防护、投递与 AES-GCM 行为均保持兼容。
