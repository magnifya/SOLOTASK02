...
不可判定落盘失败后的同进程安全自愈（仅 --data-file/E2EE_DATA_FILE 生效，内存模式不启用、行为不变）。若新快照已 `os.replace` 替换正式文件后目录 fsync 失败，且回滚重命名或回滚 fsync 也失败，则保持失败可判定：正式路径必须保持缺失，旧已提交 inode 仅保留为唯一 `.state-*.bak`，未提交新快照被移到恢复扫描绝不提升的 `.state-*.quarantine`（或删除），内存回滚到上一个已提交状态，触发该失败的请求返回 503/field=data_file；未提交快照与 `.quarantine` 在任何情况下都不得成为权威。此后同一进程收到的下一次可持久化写请求，先在存储锁内完整校验并原子提升该备份、清理同事务遗留，再只提交这一次"当前请求"本身——这里的当前请求指触发自愈的后来请求，绝不重放此前已返回 503 的旧请求（旧请求当时即已返回、不入队）。恢复候选须通过 version=1、严格等于下一次保存将写入的 commit_seq（缺一连续号即拒，legacy 无该字段按 0 且仅当下一保存写 1 时一致）、跨实体引用、消息 sequence 连续、会话级 nonce 集合、每设备 cursor 范围与 updated_at、以及 key_events 审计链的全部语义校验，并与内存中已回滚的最后已提交状态完全一致（损坏、悬空、外来、字段缺失的部分快照或两份都匹配的歧义备份一律拒绝提升；已有有效正式文件始终优先）。随后以一次 `os.replace` 原子提升并 fsync 父目录，删除该失败事务遗留的其余 `.tmp`/`.bak` 与全部 `.quarantine`；提升后再按普通事务保存当前请求，成功提交只增加一个连续 commit_seq，使设备撤销、消息/投递、同步 cursor、updated_at、key_events 与文件状态完全一致。自愈失败（备份损坏/悬空/歧义、或提升不能持久化）或当前写入再次失败：继续返回 503，正式路径仍缺失、备份仍钉住、内存与正式文件字节/inode、遗留文件与 commit_seq 均不前进，并可再次尝试；并发写经同一把存储锁线性化，只能有一个执行恢复与提交，其余请求在恢复完成后依次正常提交且各占一个连续号。重启恢复、旧文件缺字段兼容、HTTP/CLI 状态字段（503/field=data_file）、进程锁语义及无持久化模式的行为均保持不变。
...
e2ee_backend/
  ...
  persistence.py  # version=1 JSON 状态文件：缺失创建、损坏/版本不符拒启、临时文件+fsync+硬链接备份+os.replace+父目录 fsync（平台不支持时安全跳过）原子替换，启动清理/恢复崩溃遗留快照
