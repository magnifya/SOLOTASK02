...
新增提交完整性旁车（commit integrity log）。启用 --data-file/$E2EE_DATA_FILE 时，除内存 history 外的其他入口不变：在状态文件同目录维护旁车文件 **<状态文件名>.integrity**。旁车为 **UTF-8、ensure_ascii=false、紧凑无换行** JSON，键序固定 `version`、`entries`，version=1；entries 按 commit_seq 升序，每项键序固定 `commit_seq`、`state_hash`、`prev_hash`、`hash`：commit_seq 为非负整数（布尔/负数/小数/字符串拒绝），各哈希为 **64 位小写 hex**；首项 prev_hash 为空串，其后每项 prev_hash 等于前一项的 hash；state_hash 等于只读探针（GET /v1/persistence/integrity）对该代次规范快照求得的 SHA-256；hash 为把该条目**去掉 hash 项**后的对象按键排序、以同样紧凑 JSON（ensure_ascii=false、UTF-8 字节）求得的 SHA-256 小写 hex。每次成功提交在写入 commit_seq 后向旁车**追加且仅追加一条**（旁车以全量重写实现）；失败事务回滚、幂等重试不重复追加。状态文件与旁车在存储同一把锁内作为**两文件事务**提交：各自先写同目录临时文件（.state-*.tmp 与 .integrity-*.tmp）、fsync，状态文件替换在前、旁车替换在后，替换各随父目录 fsync；任一侧的临时文件 I/O、fsync 或 os.replace 失败，本次请求一律 **503/field=data_file**，内存回滚、把已替换的文件（含旁车）回滚到提交前 inode/字节，并清理全部 tmp（不可回滚的未提交旁车改放入恢复扫描永不提升的 .integrity-*.quarantine 或删除）。

状态文档顶层新增标记 **integrity_log_version**。规则：**缺标记且无旁车**（旧 version=1 文件）可以启动，首次提交在写入 commit_seq 后写入 integrity_log_version=1 及该提交后的首条旁车记录（其 prev_hash 为空——旧文件可能在任意代次才首次被本特性提交，故旁车首条 commit_seq 不要求为 0，但其后必须逐代连续）。以下情形一律**拒启**：缺标记但存在旁车、标记值≠1（含布尔/字符串/数字 0 或 2）、标记=1 但无旁车、旁车 JSON/结构错（非对象、顶层键不全或多余、version≠1、entries 非列表或为空、条目键不全或多余、commit_seq 非法、非严格升序或不连续、哈希非 64 位小写 hex）、prev_hash 链断裂、条目 hash 重算不符，或末项与状态文件不符（末项 commit_seq≠状态 commit_seq，或末项 state_hash≠状态文档规范快照的探针哈希）。拒启时 **stderr 仅一行 JSON（field=data_file）、退出码 1，状态文件与旁车原字节不改、不创建任何文件**。进程在两个替换之间崩溃（新状态已就位而旁车仍旧）时，启动恢复先保持状态文件既有权威规则，再把能完全验证且末项恰好证明当前正式状态的唯一致命 .integrity-*.tmp 提升为正式旁车并清理其余旁车遗留；没有任何可验证旁车匹配时不提升、不清理，交由上述拒启规则处理。

新增 **GET /v1/persistence/integrity/history**（无查询参数、无请求体，均忽略；不新增 CLI 子命令）。纯内存模式，以及尚无 integrity_log_version 标记的旧状态文件，返回 **409/field=data_file**（错误体键序 message、field）。成功返回 **200**，响应体键序固定 `commit_seq`、`entries`：commit_seq 为最后已提交代次，且同时等于状态文件 commit_seq 与末项 commit_seq；entries 为旁车全部条目（每项键序 commit_seq、state_hash、prev_hash、hash）。读取在存储同一把锁内与提交线性化。
...
- `GET /v1/persistence/integrity/history`：提交完整性旁车历史。纯内存模式或缺 `integrity_log_version` 标记的旧状态文件返回 `409/field=data_file`；`200` 返回键序 `commit_seq`、`entries`，commit_seq 等于最后已提交代次（同时等于状态文件与旁车末项），entries 为旁车 `<state>.integrity` 的全部条目（键序 commit_seq、state_hash、prev_hash、hash，首项 prev_hash 为空、逐代哈希链接，state_hash 为探针哈希、hash 为去 hash 项按键排序紧凑 JSON 的 SHA-256）。每次成功提交在两文件锁事务（状态文件在前、旁车在后）内追加恰好一条，任一 I/O/fsync/替换失败 503/field=data_file、回滚并清理 tmp，重试幂等不重复。缺标记无旁车可启动、首次提交写标记=1 与首项；缺标记有旁车、标记≠1、标记 1 无旁车或旁车结构/链/哈希/末项错误一律拒启（stderr 单行 JSON、field=data_file、退出码 1，原文件不改）。无 CLI 子命令。
...
e2ee_backend/
  ...
  persistence.py  # version=1 JSON 状态文件：缺失创建、损坏/版本不符拒启、临时文件+fsync+硬链接备份+os.replace+父目录 fsync（平台不支持时安全跳过）原子替换，不可判定失败后在存储锁内同进程自愈（校验并硬链接提升唯一 .bak、清理遗留、再提交当前请求；两个及以上可验证候选拒绝提升、绝不按名或 mtime 选；连正式路径都无法腾空时落 .state-*.block 标记进入阻断态、一切写 503 且不把残留正式文件当权威，路径腾出且唯一可验证备份时才硬链接提升解阻），启动清理/恢复崩溃遗留快照（同代多候选同样拒绝），提交完整性旁车 <state>.integrity（两文件锁事务追加、结构/链/哈希启动校验、崩溃后提升暂存旁车）
  ...
  http_app.py     # POST/GET 路由与 JSON 响应（注册、查询、两类撤销、会话协商与查询、群组创建/查询/成员增删、群组会话协商/查询/轮换、消息投递/幂等提交与拉取、重试/确认/状态、群会话同步与检查点、只读持久化完整性探针 /v1/persistence/integrity 与提交历史 /v1/persistence/integrity/history）
