...
e2ee_backend/
  ...
  persistence.py  # version=1 JSON 状态文件：缺失创建、损坏/版本不符拒启、临时文件+fsync+硬链接备份+os.replace+父目录 fsync（平台不支持时安全跳过）原子替换，不可判定失败后在存储锁内同进程自愈（校验并硬链接提升唯一 .bak、清理遗留、再提交当前请求），启动清理/恢复崩溃遗留快照
