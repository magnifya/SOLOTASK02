...
新增离线消息批量确认，使投递确认与同步游标在同一事务提交（1:1 与群组会话通用）。POST /v1/sessions/{session_id}/sync/ack 请求体含 device_id、cursor：device_id 须为非空字符串，cursor 须为非布尔非负整数；字段缺失或类型错误返回 400/对应 field，非对象或坏 JSON 返回 400/request_body。会话未知返回 404/session_id；设备未知、已撤销或无读取权返回 409/device_id（1:1 仅接收方可用，群组限冻结成员）；cursor 低于当前游标（无记录视为 0）或高于该会话最大 sequence 返回 409/cursor。前进时将 (old,cursor] 内可收消息逐条置 acked 并推进 message_sync_cursors：1:1 置该会话的投递记录，群组置逐设备投递记录且跳过本人所发消息，attempts 不变。响应键序 session_id、device_id、cursor、updated_at：前进返回 201 并写入 UTC ISO-8601（+00:00）时间戳，等值返回 200 且 updated_at 不变（未建游标时取会话 created_at），等值不落盘、不增 commit_seq。全部校验、逐条确认与游标推进和持久化在存储同一把锁下原子完成；落盘失败返回 503/field=data_file，游标、delivery、group_delivery、时间戳、commit_seq 与双文件一并回滚。version=1 状态文件无新增键，重启后随 message_sync_cursors 与投递段完整恢复。错误体键序 message、field。显式 after 的同步重取仍无副作用，其余接口契约不变。
...
- `POST /v1/sessions/{session_id}/sync/ack`：离线消息批量确认，投递确认与统一同步游标在同一事务提交（1:1 与群组会话通用）。体含非空 `device_id` 与非布尔非负整数 `cursor`（缺失/类型错误 `400`/对应 field，非对象或坏 JSON `400/request_body`）；会话未知 `404/session_id`，设备未知、已撤销或无读取权 `409/device_id`（1:1 仅接收方，群组限冻结成员），cursor 倒退或超最大序号 `409/cursor`。前进时将 `(old,cursor]` 内可收消息逐条置 acked（群组跳过本人所发）并推进 `message_sync_cursors`，`attempts` 不变；响应键序 `session_id`/`device_id`/`cursor`/`updated_at`，前进 `201` 刷新时间、等值 `200` 时间不变（未建游标取会话 `created_at`）且不落盘不增 commit_seq。落盘失败 `503/field=data_file` 并整体回滚；version=1 无新增键，重启完整恢复。错误体键序 `message`、`field`。
