...
- `POST /v1/devices/{device_id}/inbox/retry-batch`：重试 **1:1** 收件箱，整批在同一存储锁事务（一次持久化通知、与设备撤销共锁）内完成；路径 `device_id` 未知或已撤销 `409/field=device_id`。请求体须为对象且含非空字符串 `attempt_id` 与非空数组 `items`：坏 JSON/体非对象 `400/request_body`，`attempt_id` 缺失/空/非字符串 `400/attempt_id`，`items` 缺失/非数组/空 `400/items`；每项须为对象（非对象或 `(session_id, message_id)` 二元组重复 `400/items[i]`），含非空字符串 `session_id` 与非空字符串 `message_id`（缺失/空/非字符串分别 `400/items[i].session_id`、`400/items[i].message_id`）。设备校验后按数组顺序逐项预检、首错全不写：会话未知（既非 1:1 亦非群组会话）`404/items[i].session_id`；群会话或该设备非此 1:1 会话接收方 `409/items[i].session_id`；消息未知 `404/items[i].message_id`；消息已确认（`delivery.acked`）`409/items[i].message_id`。全部通过后在锁内向各消息 `delivery` 记录的 attempt 去重集加入本次 `attempt_id`：集合已有则不计数（同 `attempt_id` 跨消息重放只影响已含它的记录），否则 `attempts` 加 1。成功体键序 `device_id`、`results`；`results` 同输入顺序，每项键序 `session_id`、`message_id`、`attempts`。至少一条新 attempt 整批 `201`，全部重放 `200` 且不落盘、不增 `commit_seq`。整批单次提交、`commit_seq` 至多加 1；落盘失败 `503/field=data_file` 并把全部 `delivery` 变更、代次与双文件一并回滚。错误体键序 `message`、`field`。`version=1` 无新顶层键、无 CLI 子命令，重启由既有 `delivery` 段完整恢复。其余 README 入口不变且不属本题。
