...
- `GET /v1/devices/{device_id}`：返回 `identity_key`、`prekey_ids`（仅未撤销且未被领取消费，顺序与注册时一致且重复请求完全相同）、`registered_at`；不存在返回 `404`。
...
- `POST /v1/prekeys/claim`：领取预密钥。体含非空字符串 `recipient_device_id`、`claim_id`，缺失或类型错误 `400`/对应 `field`；设备未知 `404/field=recipient_device_id`，已撤销 `409`/同字段。按注册顺序选首个未撤销、未消费预密钥，原子标记已消费后 `201` 返回 `claim_id`/`recipient_device_id`/`identity_key`/`key_id`/`public_key`/`claimed_at`（UTC ISO-8601，`+00:00`）；无可用预密钥 `409/field=prekey_id`。重复 `claim_id` 返回 `200` 及首次响应且不重复消费；不同 `claim_id` 领取下一个。已消费 key 从 `GET` 的 `prekey_ids` 排除，以其创建会话返回 `409/field=prekey_id`；撤销设备或 key 后不可领取。领取、会话创建、撤销共享同一把锁线性化，竞态仅一个领取成功。领取记录存于状态文件 `prekey_claims`（含 `device_id`/`key_id` 及响应字段），旧 version=1 文件缺该段按未消费加载，段存在时校验唯一性、类型与引用，畸形拒绝启动；落盘失败 `503/field=data_file` 并回滚，重启后映射完整恢复。
- 命令行 `claim-prekey --recipient-device-id DEVICE --claim-id CLAIM` 与上述接口对应；成功（200/201）stdout 单行 JSON，失败 stderr 单行 JSON 且非零退出。
