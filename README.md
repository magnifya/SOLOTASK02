...
- `POST /v1/devices/{device_id}/inbox/leases/{lease_id}/release`：提前释放 **1:1** 收件箱租约，使其消息可在到期前被重新补领。无请求体；非空体 `400/field=request_body`。租约不存在 `404/field=lease_id`；属于其他设备 `409/field=lease_id`（两者均优先于路径设备状态判定）；首次释放时设备未知或已撤销 `409/field=device_id`（在存储锁内判定，与撤销线性化）。首次释放返回 `201`，响应键序固定 `device_id`、`lease_id`、`released_at`、`released_count`；`released_at` 为释放时刻的 UTC ISO-8601（恒为 6 位微秒、`+00:00`），`released_count` 为该租约领取消息数的非负整数。重复释放返回 `200` 并逐字节返回首次响应（即使设备此后已撤销，均为锁内优先判定）。释放后该租约不再阻挡新的 claim（释放记录保留为历史）；原 claim 以同设备同 `limit` 重放仍返回其首次响应且不重新激活。释放、领取、确认与设备撤销共用存储同一把锁线性化；首次释放一次性提交、`commit_seq` 恰好加 1，落盘失败 `503/field=data_file`，内存、状态文件与旁车（含代次、inode）一并回滚。`version=1` 不新增顶层键，仅扩展 `delivery[].leases`：元素键序变为 `lease_id`、`limit`、`leased_until`、`released_at`；旧文件缺该键按 `null` 加载，非 `null` 须符合上述时间格式（UTC、6 位微秒、`+00:00`），且同一 `lease_id` 在所有记录中须同值；类型错误或跨记录不一致一律拒绝启动且绝不覆盖原文件。序列化沿用 README 既有口径（UTF-8、`ensure_ascii=false`、紧凑 JSON，元素键序固定）；错误体键序 `message`、`field`。无新 CLI 子命令；其余 README 入口行为不变。
...
e2ee_backend/
  ...
  models.py       # Device / SignedPreKey / KeyEvent / Session / Group / GroupSession / GroupSessionRotation / GroupSyncCursor / Message / MessageSubmission / MessageDelivery（含收件箱补投租约 MessageLease） 数据模型
