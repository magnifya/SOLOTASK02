...
新增群组会话轮换。POST /v1/group-sessions/{session_id}/rotate 接收非空 rotation_id、actor_device_id、合法公钥 ephemeral_key 与正整数 expected_revision，字段缺失、类型错误、空串返回 400/对应 field，公钥非法 400/field=ephemeral_key，expected_revision 非正整数（0、负数、小数、布尔、字符串）400/field=expected_revision。前序会话未知 404/field=session_id；actor 未知 404/field=actor_device_id，actor 已撤销或不是该群组固定创建者 409/field=actor_device_id；expected_revision 与群组当前 revision 不等 409/field=expected_revision。首次轮换返回 201：后继会话获得唯一新 session_id，其 members 与 revision 按提交时群组快照冻结，响应在群会话七字段之外另含 rotation_id 与 predecessor_session_id（共九字段），前序会话状态不变。同一 rotation_id 重放同一前序返回 200 及与首次字节一致的原响应，不另建后继；同一 rotation_id 用于其他前序返回 409/field=rotation_id；前序已被其他 rotation_id 轮换后再用任何新 id 轮换一律 409/field=session_id，会话线不得分叉（后继自身可用新 id 继续轮换成链）。轮换与成员增删、撤销、消息投递及持久化共用存储同一把锁，失败不写入；落盘失败 503/field=data_file 并回滚后继与轮换记录。version=1 状态文件新增可选 group_session_rotations 段，旧文件缺该段按空加载；恢复时校验 rotation_id 唯一、前序与后继均为已存群会话且属同一群组、一前序恰有一后继（无分叉）、后继不被其他轮换复用、actor 已注册且等于后继发起者、冻结 members/revision/ephemeral_key/created_at 与后继快照完全一致，矛盾拒启且绝不覆盖原文件。后继作为普通群组会话继续按冻结成员做消息/投递鉴权（发送者本人与未注册设备仍被 group_delivery 以 409/device_id 拒绝）。命令行新增 rotate-group-session SESSION_ID --rotation-id ID --actor-device-id DEVICE_ID --ephemeral-key KEY --expected-revision N（ephemeral-key 支持既有公钥编码与 @文件），输出契约与其他命令一致。
...
- `POST /v1/group-sessions/{session_id}/rotate`：把群组会话轮换为后继会话。体含非空字符串 `rotation_id`、`actor_device_id`、合法公钥 `ephemeral_key` 与正整数 `expected_revision`，缺失/类型错误/空串 `400`/对应 `field`，`ephemeral_key` 编码非法 `400/field=ephemeral_key`，`expected_revision` 非正整数（含 0、负数、小数、布尔、字符串）`400/field=expected_revision`。前序会话未知 `404/field=session_id`；actor 设备未知 `404/field=actor_device_id`，actor 已撤销或不是该群组的固定创建者 `409/field=actor_device_id`；`expected_revision` 不等于该群组当前 `revision` `409/field=expected_revision`。首次轮换 `201`，后继为新的唯一 `session_id`，其 `members` 与 `revision` 按**提交时**群组快照冻结（冻结后群组再变更不影响后继；前序会话快照保持不变），响应在群会话七字段之上另含 `rotation_id` 与 `predecessor_session_id`（共九字段）。同一 `rotation_id` 重放同一前序返回 `200` 及与首次完全相同的原响应（即便请求携带不同临时公钥或修订），不另建后继；同一 `rotation_id` 用于其他前序返回 `409/field=rotation_id`。前序已被其他 `rotation_id` 轮换后，任何新 id 再轮换一律 `409/field=session_id`，会话线不得分叉；后继会话自身可用新 `rotation_id` 继续轮换（形成链）。轮换与成员增删、设备撤销、消息投递及持久化共享存储同一把锁、原子线性化，任一校验失败不写入。记录落于 version=1 状态文件新增 `group_session_rotations` 段（含 `rotation_id`/`session_id`/`predecessor_session_id`/`group_id`/`actor_device_id`/`ephemeral_key`/`members`/`revision`/`created_at`），旧文件缺该段按空加载；恢复时校验 rotation_id 唯一、前序与后继均为已存群会话且属同一群组、每个前序恰有一个后继（无分叉）、后继不被其他轮换复用、actor 已注册且等于后继发起者、冻结 members/revision/ephemeral_key/created_at 与后继会话快照完全一致，任何悬空引用、重复、分叉或冻结值矛盾都拒启且不覆盖原文件。落盘失败返回 `503/field=data_file` 并回滚后继会话与轮换记录（内存与文件均不前移，该 `rotation_id` 与前序稍后可重试成功）。后继会话是普通群组会话，消息与投递继续按冻结成员鉴权（发送者本人与未注册设备仍被 group_delivery 拒绝）。
- 命令行 `register` / `show` / `revoke-device` / `revoke-prekey` / `rotate-identity-key` / `add-prekey` / `claim-prekey` / `claim-user-prekeys` / `create-session` / `create-session-from-claim` / `create-batch-sessions` / `show-session` / `group-create` / `group-show` / `group-add-member` / `group-remove-member` / `create-group-session` / `show-group-session` / `rotate-group-session` / `sync-group-messages` / `sync-checkpoint` 与接口一一对应，成功时在 stdout 打印单行 JSON，字段名与 HTTP 一致；失败时在 stderr 打印单行 JSON 错误并以非零码退出。连接失败或超时时，API 命令在 stderr 打印 `field` 为 `server` 的单行 JSON、非零退出，且不输出 traceback。
- 命令行 `rotate-group-session SESSION_ID --rotation-id ID --actor-device-id DEVICE_ID --ephemeral-key KEY --expected-revision N` 与 `POST …/rotate` 一一对应（`--ephemeral-key` 支持既有公钥编码与 `@文件`）；首次成功 `201`、重放 `200` 均在 stdout 打印同一九字段单行 JSON 并以 0 退出，失败 stderr 单行 JSON 且非零退出。
...
- 持久化：`serve --data-file PATH`（缺省取 `$E2EE_DATA_FILE`，再缺省为纯内存）。状态文件 `version=1` JSON，缺失自动创建，损坏/非对象/版本不符拒绝启动；每次变更临时文件 + fsync + `os.replace` 原子替换。重启恢复设备（含 `identity_key` 与 `rotated_at`）、预密钥（含新增与撤销标记）、会话、消息与序号游标、会话级已用 nonce 集合，以及投递去重集合、`attempts`、acked 与设备撤销状态。群组与冻结群会话快照、群组会话轮换记录（`group_session_rotations`，含 `rotation_id`/`session_id`/`predecessor_session_id`/`group_id`/`actor_device_id`/`ephemeral_key`/`members`/`revision`/`created_at`）、每设备群会话同步游标（`group_sync_cursors`，含 `cursor`/`updated_at`）同样恢复；重启后默认同步从各设备保存游标继续，显式 `after` 仍只查询。缺少已用 nonce 集合字段的旧 version=1 文件可正常加载并由历史消息重建该集合；该字段存在但结构畸形仍视为损坏并拒绝启动。缺少 `group_sync_cursors` 字段的旧 version=1 文件按空加载（各设备游标视为 0）；该字段存在但记录畸形、字段缺失/类型错误、cursor 越界（含空会话）、键重复或关联（会话/设备/冻结成员）对不上时拒绝启动，且不覆盖原文件。
...
# 轮换群组会话（后继按提交时群组快照冻结；同 rotation_id 重放返回 200 原响应）
python3 -m e2ee_backend rotate-group-session SESSION_ID \
  --rotation-id rot-1 --actor-device-id laptop \
  --ephemeral-key BASE64_OR_PEM_PUBLIC_KEY --expected-revision 2
# => {"session_id":"…(新后继)","group_id":"team","initiator_device_id":"laptop",
#     "ephemeral_key":"…","revision":2,"members":["laptop","tablet"],
#     "created_at":"…","rotation_id":"rot-1","predecessor_session_id":"SESSION_ID"}
...
e2ee_backend/
  ...
  storage.py      # 线程安全的进程内存储（插入顺序、撤销过滤、原子快照、会话原子创建、群组与冻结群会话、消息原子追加与分页（群组会话限冻结成员）、投递去重/确认、群会话按设备同步分页与检查点游标、整体状态快照与恢复）
  persistence.py  # version=1 JSON 状态文件：缺失创建、损坏/版本不符拒启、临时文件+fsync+硬链接备份+os.replace+父目录 fsync 原子替换，启动清理/恢复崩溃遗留快照
  service.py      # 业务逻辑与字段校验（400/404/409，设备/预密钥/会话/群组/群会话/消息/投递/群同步）
  http_app.py     # POST/GET 路由与 JSON 响应（注册、查询、两类撤销、会话协商与查询、群组创建/查询/成员增删、群组会话协商与查询、消息投递与拉取、重试/确认/状态、群会话同步与检查点）
