...
- `POST /v1/sessions/from-batch-claim`：凭一次已成功的用户批量预钥领取，原子地为领取中的每台设备各建立一个会话（单设备 `from-claim` 契约不变）。请求体含 `claim_id`、`initiator_device_id`（均须为非空字符串，否则 `400/field=对应字段`）与 `ephemeral_keys`：非空数组，每项为含非空且在数组内唯一的 `device_id` 与合法公钥 `ephemeral_key` 的对象；数组缺失、非数组、为空、元素非对象、子字段缺失/空、公钥编码非法、`device_id` 重复均返回 `400/field=ephemeral_keys[下标].device_id`（或 `.ephemeral_key`；数组级问题为 `ephemeral_keys`）。`ephemeral_keys` 的设备集合必须与领取快照完全相等（一台一项、不多不少），否则 `400/field=ephemeral_keys`。`claim_id` 无对应领取返回 `404/field=claim_id`；该 id 属于一条单领记录返回 `409/field=claim_id`；已用本接口建立过会话（已绑定）同样 `409/field=claim_id`。发起设备未知返回 `404/field=initiator_device_id`，已撤销返回 `409/同字段`；若发起设备本身就在领取快照的设备集合中，整批返回 `400/field=initiator_device_id` 且不创建任何会话（即使该设备此后已撤销仍为 `400`）。领取快照中某接收设备已撤销（或已不存在）返回 `409/field=recipient_device_id`，其冻结预钥已撤销返回 `409/field=prekey_id`，按领取顺序定位首个出错项；任一校验失败整批不写入。成功 `201` 返回 `claim_id` 与 `sessions`：`sessions` 按领取的注册顺序列出，每个为现有八字段会话快照，`recipient_device_id`/`prekey_id`/`identity_key`/`public_key` 取领取冻结值（接收设备此后轮换身份密钥不改变），`initiator_device_id` 与该设备对应的 `ephemeral_key` 取请求值，`session_id`/`created_at` 按现有契约生成。每个批量 `claim_id` 至多成功建立一批会话，重复提交一律 `409/field=claim_id`。重复绑定检查、设备集合校验、全部状态校验、全部会话写入与批量绑定在存储同一把锁内一次提交：全成或全败，与领取、撤销、其它会话创建线性化。批量绑定写入 version=1 状态文件（`batch_claim_session_bindings` 段）并随重启恢复，旧文件缺该段按空加载；落盘失败返回 `503/field=data_file` 并回滚全部会话与绑定（内存与文件均不前移，该批量 `claim_id` 未被占用，稍后可整批重试成功）。
...
e2ee_backend/
  ...
  cli.py          # register/show/revoke-*/rotate-identity-key/add-prekey/claim-prekey/claim-user-prekeys/create-session/create-session-from-claim/create-batch-sessions/show-session/group-*/create-group-session/show-group-session/sync-group-messages/sync-checkpoint/send-message/pull-messages/retry-message/ack-message/message-status/encrypt-message/decrypt-message/serve 命令行入口
tests/            # unittest 测试
```
