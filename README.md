...
新增用户多设备预钥批量领取，既有接口契约不变。POST /v1/prekeys/claim-batch 请求体含非空字符串 user_id、claim_id，缺失或类型错误返回 400 并以 field 指明对应字段；用户不存在返回 404/field=user_id；该用户没有任何未撤销设备返回 409/field=device_id。按注册顺序枚举该用户全部未撤销设备（已撤销设备跳过），每台取首个未撤销且未消费预钥；只要任一设备没有可用项即返回 409/field=prekey_id 且所有设备的预钥均不消费（失败前后状态完全一致）。全部可用时在存储同一把锁内一次性标记消费并持久化，201 返回 claim_id、user_id、claimed_at（UTC ISO-8601，+00:00）与 devices；devices 按上述设备顺序排列，每项含 device_id、identity_key、key_id、public_key，均为领取时冻结值，此后身份轮换或撤销不改变重复响应。重复相同批量 claim_id 返回 200 及与首次完全相同的完整响应，不再消费任何预钥。选择与消费和单领、设备/预钥撤销、补钥、会话创建共享同一把锁线性化，任何校验失败都不改变状态。批量记录写入 version=1 状态文件新段 prekey_batch_claims，旧文件缺该段按空加载；恢复时校验批次 claim_id 唯一、批次内设备不重复且按注册顺序排列、每个设备确实属于该 user_id、预钥归属该设备、冻结 public_key 与存储一致，以及冻结项与预钥 consumed 标记及既有 prekey_claims 记录两两一致（同一 device/key 不得同时出现在两条领取记录中），任一矛盾拒绝启动且不覆盖原文件。落盘失败返回 503/field=data_file 并回滚（内存与文件均不前移）。命令行新增 claim-user-prekeys，参数 --user-id、--claim-id，沿用成功 stdout、失败 stderr 单行 JSON 与非零退出契约（201/200 均为成功，连接失败 field=server）。
...
- `POST /v1/prekeys/claim-batch`：按用户批量领取预钥（单领接口契约不变）。体含非空字符串 `user_id`、`claim_id`，缺失或类型错误 `400`/对应 `field`；用户不存在 `404/field=user_id`；无未撤销设备 `409/field=device_id`。按注册顺序枚举该用户全部未撤销设备，每台取首个未撤销、未消费 key；任一设备无可用项即 `409/field=prekey_id` 且全部不消费。全部可用时锁内一次性消费并持久化，`201` 返回 `claim_id`/`user_id`/`claimed_at`/`devices`，`devices` 按上述顺序、每项含 `device_id`/`identity_key`/`key_id`/`public_key`（冻结值，UTC ISO-8601 `+00:00`）。重复批量 `claim_id` 返回 `200` 及首次完整响应、不再消费。选择、消费与单领、撤销、补钥、会话创建共锁线性化，任何失败不改状态。记录落于状态文件 `prekey_batch_claims` 段，旧文件缺该段按空加载；恢复时校验批次唯一、设备属于用户且按注册顺序、预钥归属、冻结值与消费记录一致，矛盾拒启。落盘失败 `503/field=data_file` 并回滚。命令行新增 `claim-user-prekeys --user-id … --claim-id …`，成功 stdout、失败 stderr 单行 JSON 且非零退出。
...
e2ee_backend/
  ...
  models.py       # Device / SignedPreKey / PreKeyClaim / PreKeyBatchClaim / Session / ClaimSessionBinding / Group / GroupSession / GroupSyncCursor / Message / MessageDelivery 数据模型
  storage.py      # 线程安全的进程内存储（插入顺序、撤销过滤、原子快照、会话原子创建、群组与冻结群会话、消息原子追加与分页（群组会话限冻结成员）、投递去重/确认、群会话按设备同步分页与检查点游标、单领与用户多设备批量领取（全有或全不消费、幂等重放）、整体状态快照与恢复）
  ...
  http_app.py     # POST/GET 路由与 JSON 响应（注册、查询、两类撤销、单领与 /v1/prekeys/claim-batch 批量领取、会话协商与查询、群组创建/查询/成员增删、群组会话协商与查询、消息投递与拉取、重试/确认/状态、群会话同步与检查点）
