...
- `GET /v1/group-sessions/{session_id}/sync`：按设备增量同步群组会话消息。查询参数 `device_id` 单值非空（缺失/为空/重复 `400/field=device_id`）；`after` 省略时从该设备已保存游标起步并在锁内推进到本页末条（空页不推进），显式给出时须为非负整数、仅一次性查询且不读取也不改写已保存游标（非法 `400/field=after`）；`limit` 默认 100（1..100，越界非法 `400/field=limit`）。未知会话 `404/session_id`，设备未知、已撤销或不在冻结成员表（冻结后才加入者）均 `409/device_id`；冻结后被移出当前群组者仍可同步。恒 `200` 返回 `messages`（信封数组，按 POST 写入/sequence 严格升序、无跳号重复）、`next_cursor`（末条序号，空页为起点，连续请求中与 `has_more` 保持一致）、`has_more`（起点之后是否还有消息）。启用 `--data-file` 时重启后默认同步从保存的 cursor 续传，各设备游标独立。
- `POST /v1/group-sessions/{session_id}/sync/checkpoint`：推进设备同步检查点。体含非空 `device_id`（非法 `400/device_id`）与整数 `cursor`（缺失/非整数/布尔/负数 `400/cursor`），且 `cursor` 不超过该会话最大 sequence（空会话仅允许 0）。前进 `201` 并刷新 `updated_at`，相同 `200` 且 `updated_at` 不变，倒退或超过最大序号 `409/field=cursor`；初始 cursor=0 的确认在游标从未推进过时返回 `200`。返回 `session_id`/`device_id`/`cursor`/`updated_at`。未知会话 `404/session_id`；设备未知、已撤销或非冻结成员均 `409/device_id`。鉴权、分页与游标推进共享存储同一把锁，失败不改变消息游标。游标存于状态文件 `group_sync_cursors`，重启后 cursor 与 `updated_at` 原样恢复；旧 version=1 文件缺该字段按空（游标 0）加载，该字段存在但记录、类型、范围、重复键或关联结构畸形（引用未知群组会话/未知设备/非冻结成员、cursor 超过会话最大 sequence 等）或版本不符均拒绝启动且不覆盖原文件。默认同步推进、检查点前进与设备撤销在同一事务边界内与持久化一起线性化：临时文件写入、fsync 或 `os.replace` 失败返回 `503/field=data_file`，内存回滚、旧文件保留、临时文件清理；未启用 `--data-file` 时行为不变。
...
- 持久化：`serve --data-file PATH`（缺省取 `$E2EE_DATA_FILE`，再缺省为纯内存）。状态文件 `version=1` JSON，缺失自动创建，损坏/非对象/版本不符拒绝启动；每次变更临时文件 + fsync + `os.replace` 原子替换。重启恢复设备（含 `identity_key` 与 `rotated_at`）、预密钥（含新增与撤销标记）、会话、消息与序号游标、会话级已用 nonce 集合，以及投递去重集合、`attempts`、acked 与设备撤销状态。群组与冻结群会话快照、每设备群会话同步游标（`group_sync_cursors`，含 `cursor`/`updated_at`）同样恢复。缺少已用 nonce 集合字段的旧 version=1 文件可正常加载并由历史消息重建该集合；该字段存在但结构畸形仍视为损坏并拒绝启动。缺少 `group_sync_cursors` 字段的旧 version=1 文件按空加载（各设备游标视为 0）；该字段存在但记录、类型、范围、重复键或关联结构畸形（引用未知群组会话/未知设备/非冻结成员、cursor 超过会话最大 sequence）同样拒绝启动且不覆盖原文件。默认同步推进、检查点前进与设备撤销（及其余一切变更）在事务边界内与文件原子替换一起线性化：临时文件写入、fsync 或 `os.replace` 失败时返回 `503/field=data_file`，内存恢复变更前快照、旧文件原样保留、临时文件清理，失败不推进内存或文件；纯内存部署永不出现该错误。
