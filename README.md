...
新增设备密钥审计：为每台设备维护一条按 device_id 隔离、只增不改、哈希链接的密钥事件链。事件字段固定为 device_id、seq、type、payload、prev_hash、hash、created_at；每台设备 seq 从 1 起连续，首条 prev_hash 为空串，其后每条的 prev_hash 等于前一条的 hash。各类型 payload：registered 为 {"prekeys":[{"key_id","public_key"}, …]}（即注册体中按提交顺序排列的预钥），identity_rotated 为 old_identity_key/new_identity_key，prekey_added 与 prekey_revoked 为 key_id+公钥，device_revoked 为空对象（表示全部撤销）。hash 为"去掉 hash 字段后、键排序、紧凑分隔、不转义非 ASCII（Unicode 原样）的 JSON"的 UTF-8 SHA-256 小写 hex。注册、身份轮换、新预钥、预钥或设备撤销在同一存储锁事务内追加对应事件；幂等操作（相同身份密钥、重复预钥 id/公钥、重复撤销）与任何失败（校验失败、404/409、落盘失败回滚）一律不追加。落盘失败仍返回 503/field=data_file 并回滚内存与文件，链不前进。GET /v1/devices/{device_id}/key-events：after 默认 0 且必须 ≥0，limit 默认 100、取值 1..100；缺失/非整数/越界/重复给出返回 400/对应 field（after 或 limit）；未知设备 404/field=device_id；已撤销设备仍可读取完整链。恒返回 200，体含 events（seq>after 的前 limit 条，按 seq 升序）、next_after（空页等于 after）、has_more。命令行新增 key-events DEVICE_ID [--after N] [--limit N]，成功 stdout 单行 JSON，失败 stderr 单行 JSON 并非零退出，连接失败 field=server。链落于 version=1 可选段 key_events：旧文件缺省按空（该进程的审计保持关闭，后续变更继续不写该段，保证本服务写出的文件总能被自己恢复）；该段存在时恢复须逐事件校验字段齐全与类型、device 引用、每设备 seq 从 1 连续、prev_hash 链接与重算 hash 一致，链集合必须恰好覆盖全部已注册设备，并按顺序重放核对身份密钥（首次轮换的 old_identity_key 锚定注册身份、其后轮换 old→new 必须衔接）、预钥顺序与公钥、单钥与设备撤销状态，与 devices 段任一矛盾即拒启且不覆盖原文件。
