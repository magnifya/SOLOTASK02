...
- `POST /v1/sessions/from-claim`：从预密钥领取建立会话（`POST /v1/sessions` 保持不变）。体含 `claim_id`、`initiator_device_id`、`ephemeral_key` 三个非空字符串，`ephemeral_key` 沿用公钥编码，缺失、类型或编码错误 `400/field=对应字段`。`claim_id` 未知 `404/field=claim_id`；发起设备未知 `404`、已撤销 `409`（均 `field=initiator_device_id`）；领取关联的接收设备或预钥已撤销分别 `409/field=recipient_device_id`、`409/field=prekey_id`。成功 `201` 返回与现有会话相同的八字段，其中 `recipient_device_id`、`prekey_id`、`identity_key`、`public_key` 取领取时冻结值，其余按现有会话契约生成。每个 `claim_id` 只能成功建立一次会话，重复提交均 `409/field=claim_id` 且不新建。绑定、状态校验、会话写入与撤销共享存储同一把锁线性化，失败不占用领取；绑定写入 version=1 状态文件 `claim_sessions` 段并随重启恢复，旧文件缺该段按空加载；落盘失败 `503/field=data_file` 并回滚绑定与会话。命令行新增 `create-session-from-claim`（`--claim-id` / `--initiator-device-id` / `--ephemeral-key`），沿用单行 JSON 与退出码契约。
