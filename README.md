...
serve 支持持久化：--data-file 指定状态文件路径，缺省时取环境变量 E2EE_DATA_FILE，二者皆无则保持进程内存储。状态文件为 version=1 的单个 JSON 文档：文件缺失时自动创建；文件损坏、非 JSON 对象或版本不符时拒绝启动（不丢弃既有状态）。文档顶层带提交代次 commit_seq：首次创建的空状态写入 commit_seq=0，此后每次成功持久化的事务（设备、群组、消息、同步、投递、撤销或审计链变更）只递增一次且严格连续；临时文件写入、文件 fsync、os.replace 或真实目录 fsync 错误均返回 503/field=data_file，内存、代次、文件字节与 inode 一并回滚并清理遗留，代次不跳号；平台或文件系统不支持目录 fsync 时安全跳过，不误报失败。启动时旧 version=1 文件缺 commit_seq 按 0 兼容；该字段存在时必须是非负整数，布尔、负数、小数或字符串均拒绝启动（stderr 单行 JSON、field=data_file、退出码 1，原文件不变）。每次变更先写同目录临时文件、fsync 文件、替换前以同目录硬链接（.state-*.bak）钉住旧 inode、os.replace 原子替换并 fsync 父目录，使重命名本身也抗崩溃，崩溃不会留下半写文件或未完成的重命名。重启后完整恢复且游标、消息、撤销、投递与审计链不回退，下一次提交在恢复到的代次上继续递增：设备与预密钥（含撤销标记）、会话快照、消息与序号游标、会话级已用 nonce 集合（旧文件缺该字段时由历史消息重建），以及可靠投递的尝试去重集合、attempts、acked 状态与撤销状态。
